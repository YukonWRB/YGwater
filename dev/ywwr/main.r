...
conn <- AquaCache::AquaConnect(name = config$dbName, 
                   host = config$dbHost, 
                   port = config$dbPort, 
                   user = config$dbUser, 
                   pass = config$dbPass)
...
split_pdf_to_pages <- function(pdf_path, output_dir = tempdir()) {
    ...
    module_data <- reactiveValues(
        drillers = DBI::dbGetQuery(conn, "SELECT * FROM boreholes.drillers")
    )
...
observe({
       updateSelectizeInput(
        session, 
        "drilled_by", 
        choices = stats::setNames(module_data$drillers$driller_id, module_data$drillers$name),
    ...
    observeEvent(input$drilled_by, {
        ...
        existing_driller_ids <- module_data$drillers$driller_id
        existing_driller_names <- module_data$drillers$name
    ...
    observeEvent(input$save_new_driller, {
        ...
        query <- paste0(
            "INSERT INTO drillers (name, address, phone, email) ",
            "VALUES('", new_driller_name, "', ", 
            ...
            "RETURNING driller_id"
        )

        new_driller_id <- DBI::dbGetQuery(conn, query)[1,1]

        module_data$drillers <- DBI::dbGetQuery(conn, "SELECT * FROM boreholes.drillers")
      ...
      observe({
        ...
        if (!is.na(max_value) && max_value > 0) {
          ...
          if (length(max_index) > 0) {
            ...
            # Debug output
            cat("Updating field:", field_name, "with text:", selected_text, "\n")
            ...
            if (field_name %in% c("name", "notes", "location_source")) {
              ...
              runjs(sprintf("var el=$('#%s'); if(el.length){el.addClass('flash-update'); setTimeout(function(){el.removeClass('flash-update');},1400);}", field_name))
              ...
              tryCatch({
                ...
                cat("Error in numeric extraction:", e$message, "\n")
                showNotification("Error extracting numeric value", 
              ...
              tryCatch({
                  ...
                  if (!is.na(date_match) && date_match > 0) {
                    ...
                    if (length(date_str) > 0) {
                      ...
                      if (!is.na(parsed_date)) {
                        ...
                        runjs("var el=$('#date_drilled'); if(el.length){el.addClass('flash-update'); setTimeout(function(){el.removeClass('flash-update');},1400);}")
                ...
                cat("Date parsing error:", e$message, "\n")
                showNotification("Error parsing date", type = "error", 
                                 duration = 2)
              })
            } else {
            ...
            if (well_id %in% names(rv$well_data)) {
                ...
                tryCatch({
                    ...
                    result <- AquaCache::insertACBorehole(
                        ...
                        purpose_of_well = metadata$purpose_of_well,
                        depth_to_bedrock = metadata$depth_to_bedrock,
                        permafrost_present = metadata$permafrost_present %||% FALSE,
                        permafrost_top_depth = metadata$permafrost_top_depth,
                        permafrost_bottom_depth = metadata$permafrost_bottom_depth,
                        date_drilled = metadata$date_drilled,
                        casing_outside_diameter = metadata$casing_outside_diameter,
                        is_well = metadata$is_well %||% FALSE,
                        well_depth = metadata$drill_depth,
                        top_of_screen = metadata$top_of_screen,
                        bottom_of_screen = metadata$bottom_of_screen,
                        well_head_stick_up = metadata$well_head_stick_up,
                    ...
                    cat("Error uploading borehole", well_id, ":", e$message, "\n")
                })
            }
        }
