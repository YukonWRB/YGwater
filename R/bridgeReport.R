...
#'
#' @param database If using a local database created using HydroMetDB package, specify its path here or use 'default' to use the hydrometConnect default path. Leave NULL to download from Aquarius instead.
                         ...
                         )
{
  ...
  if (!is.null(database)){
    database <- hydrometConnect(path = database)
