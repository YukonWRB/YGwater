...
tabularReport <- function(level_locations = "all", flow_locations = "all", snow_locations = "all", bridge_locations = "all", precip_locations = "default", past = 28, save_path = "choose", archive_path = "choose", con = NULL) {
  ...
  if (!is.null(level_locations)) {
    if (level_locations[1] == "default") {
      ...
      level_locations <- DBI::dbGetQuery(con, "SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'water level' ORDER BY location;")
    } else {
      level_locations <- DBI::dbGetQuery(con, paste0("SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'water level' AND t.location IN ('", paste(level_locations, collapse = "', '"), "') ORDER BY location;"))
    }
  }
  ...
  if (!is.null(flow_locations)) {
    if (flow_locations[1] == "default") {
      ...
      flow_locations <- DBI::dbGetQuery(con, paste0("SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'discharge, river/stream' AND t.location IN ('", paste(flow_locations, collapse = "', '"), "') ORDER BY location;"))
    } else if (flow_locations[1] == "all") {
      flow_locations <- DBI::dbGetQuery(con, "SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'discharge, river/stream' ORDER BY location;")
    } else {
      flow_locations <- DBI::dbGetQuery(con, paste0("SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'discharge, river/stream' AND t.location IN ('", paste(flow_locations, collapse = "', '"), "') ORDER BY location;"))
    }
  }
  ...
  if (!is.null(snow_locations)) {
    if (snow_locations[1] == "default") {
      ...
      snow_locations <- DBI::dbGetQuery(con, paste0("SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'snow water equivalent' AND t.location IN ('", paste(snow_locations, collapse = "', '"), "') ORDER BY location;"))
    } else if (snow_locations[1] == "all") {
      snow_locations <- DBI::dbGetQuery(con, "SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'snow water equivalent' ORDER BY location;")
    } else {
      snow_locations <- DBI::dbGetQuery(con, paste0("SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'snow water equivalent' AND t.location IN ('", paste(snow_locations, collapse = "', '"), "') ORDER BY location;"))
    }
  }
  ...
  if (!is.null(bridge_locations)) {
    if (bridge_locations[1] == "default") {
      ...
      bridge_locations <- DBI::dbGetQuery(con, paste0("SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'distance' AND t.location IN ('", paste(bridge_locations, collapse = "', '"), "') ORDER BY location;"))
    } else if (bridge_locations[1] == "all") {
      bridge_locations <- DBI::dbGetQuery(con, "SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'distance' ORDER BY location;")
    } else {
      bridge_locations <- DBI::dbGetQuery(con, paste0("SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'distance' AND t.location IN ('", paste(bridge_locations, collapse = "', '"), "') ORDER BY location;"))
    }
  }
  ...
  if (!is.null(precip_locations)) {
    if (precip_locations[1] == "default") {
      ...
      precip_locations <- DBI::dbGetQuery(con, paste0("SELECT t.location FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name IN ('water level', 'discharge, river/stream') AND t.location IN ('", paste(precip_locations, collapse = "', '"), "') ORDER BY location;"))[,1]
      precip_locations <- unique(precip_locations)
    } else if (precip_locations[1] == "all") {
      precip_locations <- DBI::dbGetQuery(con, "SELECT t.location FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name IN ('water level', 'discharge, river/stream') ORDER BY location;")[,1]
      precip_locations <- unique(precip_locations)
    }
  }
