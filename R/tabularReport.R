...
tabularReport <- function(level_locations = "all", flow_locations = "all", snow_locations = "all", bridge_locations = "all", precip_locations = "default", past = 28, save_path = "choose", archive_path = "choose", con = NULL) {
  ...
  if (!is.null(flow_locations)) {
    if (flow_locations[1] == "default") {
      ...
      flow_locations <- DBI::dbGetQuery(con, "SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'flow' ORDER BY location;")
    } else {
      flow_locations <- DBI::dbGetQuery(con, paste0("SELECT t.location, t.timeseries_id FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name = 'flow' AND t.location IN ('", paste(flow_locations, collapse = "', '"), "') ORDER BY location;"))
    }
  }
  ...
  if (!is.null(precip_locations)) {
    if (precip_locations[1] == "default") {
      ...
      precip_locations <- DBI::dbGetQuery(con, paste0("SELECT t.location FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name IN ('water level', 'flow') AND t.location IN ('", paste(precip_locations, collapse = "', '"), "') ORDER BY location;"))[,1]
      precip_locations <- unique(precip_locations)
    } else if (precip_locations[1] == "all") {
      precip_locations <- DBI::dbGetQuery(con, "SELECT t.location FROM timeseries AS t JOIN parameters AS p ON t.parameter_id = p.parameter_id WHERE p.param_name IN ('water level', 'flow') ORDER BY location;")[,1]
      precip_locations <- unique(precip_locations)
    }
  }
