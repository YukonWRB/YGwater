...
learnR <- function(lesson = NULL) {
  ...
  if (interactive()) {
    ...
    if (Sys.getenv("RSTUDIO") == "1") {
      ...
      stop(
        "This training course is designed to work with RStudio. Please open the file in RStudio."
      )
    }
  } else {
    # Open the file in a text editor
    stop(
      "This training course must be used in an interactive session, in RStudio. Please open the file in RStudio."
    )
  }
...
learnR_hint <- function(lesson, part = 1) {
  ...
  if (lesson == 1) {
    cat(
      "Hint: the console is usually the bottom-left pane and is where you would have just type in a 'y', and a library call looks like this:  library(packageName).\n"
    )
  } else if (lesson == 2) {
    if (part == 1) {
      cat(
        "You call a function by typing its name (with pacakgeName:: first if you haven't done a library call) in the console, followed by parantheses. If there are arguments they go in the parentheses, but for swirl there are no arguments. Just swirl::swirl() will do!"
      )
    } else {
    ...
    if (part == 1) {
      ...
      cat(
        "Hint: you can generate a sequence of dates using the seq.Date() function. For example, seq.Date(from = as.Date('2023-01-01'), to = as.Date('2023-01-10'), by = 'day') will give you a sequence of dates from January 1, 2023 to January 10, 2023.\n"
      )
      ...
      if (tolower(response) == "y") {
        cat(
          "Hint: You can create a sequence of numbers from 1 to 10, increasing by 1 using seq(from = 1, to = 1, by = 1).\n"
        )
      } else {
        return()
      }
      ...
      if (tolower(response) == "y") {
        cat(
          "Hint: You can use the sin() function to generate sine values. For example, sin(seq(0, 2*pi, length.out = 10)) will give you 10 sine values between 0 and 2*pi.\n"
        )
      } else {
        return()
      }
      ...
      if (tolower(response) == "y") {
        cat(
          "Hint: With 365 days in 2023, you'll need 365 corresponding entries in the 'values' column. Look at the length.out argument of the seq() function.\n"
        )
      } else {
        return()
      }
      ...
      cat(
        "The assignment operator is '<-' in R, and it is used to assign values to variables. For example, x <- 5 assigns the value 5 to the variable x. Once x becomes a variable, you can assign it to a new variable, as in y <- y\n"
      )
    }
  } else if (lesson == 4) {
...
learnR_answer <- function(lesson, part = 1) {
  ...
  if (lesson == 1) {
    cat(
      "Answer: In the R console window (where you're reading this message), type in  library('YGwater'). I've added this to your console to help you out, you just need to press 'Enter'.\n"
    )
    ...
    if (part == 1) {
      cat(
        "Answer: Use df <- data.frame() to create a data.frame, and assign columns as such: data.frame(date = ???, value = ???). The date sequence can be made as such: seq.Date(from = as.Date('2023-01-01'), to = as.Date('2023-12-31'), by = 'day'), and the value sequence can be made as such: sin(seq(0, 2*pi, length.out = 365))\n"
      )
      rstudioapi::sendToConsole(
        "df  <- data.frame(date = seq.Date(from = as.Date('2023-01-01'), to = as.Date('2023-12-31'), by = 'day'),\nvalue = sin(seq(0, 2*pi, length.out = 365)))",
        execute = FALSE
      )
    } else if (part == 2) {
      cat(
        "Answer: df$value[1:30] means 'take df_value and select rows 1 to 30 (inclusively)'. We need the mean of that, so...\n"
      )
