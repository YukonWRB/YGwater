...
#'
#' A pared-down, sped up, simplified version of the original ggplot 'plotOverlap' function. Created for use with a simple Shiny application linked to the Flood Atlas, but will likely see other uses.
#'
...
plotOverlap_shiny <- function(location,
                              sub_location = NULL,
                              parameter,
                              record_rate = NULL,
                              startDay = 1,
                              ...
                              lang = "en")
{
  ...
  lubridate::year(daily_start) <- min(years)
  daily <- DBI::dbGetQuery(con, paste0("SELECT date, value, max, min, q75, q25 FROM measurements_calculated_daily_corrected WHERE timeseries_id = ", tsid, " AND date >= '", daily_start, "'AND date <= '", daily_end, "' ORDER by date ASC;"))
