...
EQWinReport <- function(
  ...
  if (is.null(stations) & is.null(stnGrp)) {
    stop("You must specify either stations or stnGrp")
  }
  if (!is.null(stations) & !is.null(stnGrp)) {
    stop("You must specify either stations or stnGrp, not both")
  }
  ...
  if (length(date) > 1) {
    stop("date must be a single date")
  }
  if (inherits(date, "Date")) {
    date <- as.character(date)
  } else if (!inherits(date, "character")) {
    stop(
      "date must be a Date object or a character vector that can be coerced to a date"
    )
  }
  ...
  tryCatch(
    ...
    error = function(e) {
      stop(
        "Failed to convert 'date' parameter to a Date object. Please provide a valid date in the format 'YYYY-MM-DD'"
      )
    }
  )

  if (!inherits(date_approx, "numeric")) {
    stop("date_approx must be a numeric value")
  }
  if (date_approx < 0) {
    stop("date_approx must be a positive number")
  }

  # Check the SD_exceed related parameters
  if (!is.null(SD_exceed)) {
    if (!is.numeric(SD_exceed)) {
      stop("SD_exceed must be a numeric value")
    }
    if (SD_exceed < 0) {
      stop("SD_exceed must be a positive number")
    }

    if (!is.null(SD_start)) {
      if (length(SD_start) > 1) {
        stop("SD_start must be a single date")
      }
      if (inherits(SD_start, "Date")) {
        SD_start <- as.character(SD_start)
      } else if (!inherits(SD_start, "character")) {
        stop(
          "SD_start must be a Date object or a character vector that can be coerced to a date"
        )
      }
      tryCatch(
        {
        ...
        error = function(e) {
          stop(
            "Failed to convert 'SD_start' parameter to a Date object. Please provide a valid date in the format 'YYYY-MM-DD'"
          )
        }
      )
    }
    if (!is.null(SD_end)) {
      if (length(SD_end) > 1) {
        stop("SD_end must be a single date")
      }
      if (inherits(SD_end, "Date")) {
        SD_end <- as.character(SD_end)
      } else if (!inherits(SD_end, "character")) {
        stop(
          "SD_end must be a Date object or a character vector that can be coerced to a date"
        )
      }
      tryCatch(
        {
        ...
        error = function(e) {
          stop(
            "Failed to convert 'SD_end' parameter to a Date object. Please provide a valid date in the format 'YYYY-MM-DD'"
          )
        }
      )
    }
    if (!is.null(SD_doy)) {
      if (!is.numeric(SD_doy)) {
        stop("SD_doy must be a numeric vector")
      }
      if (length(SD_doy) == 0) {
        stop("SD_doy must have at least one value")
      }
      if (any(SD_doy < 1) | any(SD_doy > 366)) {
        stop("SD_doy values must be between 1 and 366")
      }
    }
  }
  ...
  if (!is.null(stnGrp)) {
    ...
    grp_count <- DBI::dbGetQuery(
      con,
      paste0(
        "SELECT COUNT(*) FROM eqgroups WHERE groupname = '",
        stnGrp,
        "' AND dbtablename = 'eqstns'"
      )
    )[1, 1]
    if (grp_count == 0) {
      stop(
    ...
    StnIds <- DBI::dbGetQuery(
      con,
      paste0(
        "SELECT groupitems FROM eqgroups WHERE groupname = '",
        stnGrp,
        "' AND dbtablename = 'eqstns'"
      )
    )$groupitems
  ...
  if (!is.null(stations)) {
    StnIds <- DBI::dbGetQuery(
      con,
      paste0(
        "SELECT StnId, StnCode FROM eqstns WHERE StnCode IN ('",
    ...
    if (nrow(StnIds) == 0) {
      stop(
        "No stations found in the EQWin database with the names '",
        paste0(stations, collapse = "', '"),
        "'"
      )
    }
    if (nrow(StnIds) < length(stations)) {
      ...
      warning(
        "The following stations were not found in the EQWin database: ",
  ...
  if (!is.null(parameters)) {
    ParamIds <- DBI::dbGetQuery(
      con,
      paste0(
        "SELECT ParamId, ParamCode FROM eqparams WHERE ParamCode IN ('",
  ...
  if (!is.null(stds)) {
    standards <- DBI::dbGetQuery(
      con,
      paste0(
        "SELECT StdId, StdCode, StdFlag, StdDesc FROM eqstds WHERE StdCode IN ('",
        paste0(stds, collapse = "', '"),
        "')"
      )
    )
    if (nrow(standards) == 0) {
      stop(
        "No standards found in the EQWin database with the names '",
        paste0(stds, collapse = "', '"),
        "'"
      )
    }
    if (nrow(standards) < length(stds)) {
      ...
      warning(
        "The following standards were not found in the EQWin database: ",
        paste0(missing, collapse = ", ")
      )
    }
  }
  ...
  sampleIds <- DBI::dbGetQuery(
    con,
    paste0(
      "SELECT StnId, SampleId, CollectDateTime FROM eqsampls WHERE StnId IN (",
      paste0(StnIds, collapse = ", "),
      ") AND DateValue(CollectDateTime) = '",
      date,
      "' AND SampleClass <> 'D';"
    )
  )
  ...
  params <- DBI::dbGetQuery(
    con,
    paste0(
      "SELECT ParamId, ParamCode, ParamName FROM eqparams WHERE ParamId IN (",
  ...
  locations <- DBI::dbGetQuery(
    con,
    paste0(
      "SELECT StnId, StnCode, StnName, StnDesc FROM eqstns WHERE StnId IN (",
  ...
  samps_locs$colnames <- c(paste0(
    samps_locs$StnName,
    " (",
    samps_locs$CollectDateTime,
    ")"
  ))
  for (i in unique(samps_locs$colnames)) {
    ...
    if (length(iter) > 1) {
      count <- 2
      for (j in iter[-1]) {
        samps_locs$colnames[j] <- paste0(
          samps_locs$colnames[j],
          " (",
          count,
          ")"
        )
  ...
  for (i in 1:nrow(samps_locs)) {
    tmp <- data.frame(
      tmp = results[results$SampleId == samps_locs$SampleId[i], "Result"],
      Parameter = results[
        results$SampleId == samps_locs$SampleId[i],
        "ParamName"
      ],
      check.names = FALSE
    )
  ...
  if (!is.null(stds)) {
    stdVals <- DBI::dbGetQuery(
      con,
      paste0(
        "SELECT StdId, ParamId, MaxVal, MinVal FROM eqstdval WHERE StdId IN (",
        paste(standards$StdId, collapse = ", "),
        ") AND ParamId IN (",
        paste0(params$ParamId, collapse = ", "),
        ");"
      )
    )
    ...
    for (i in 1:nrow(standards)) {
      ...
      for (j in 1:nrow(stdivals)) {
        ...
        string <- paste0(
          if (is.na(min)) "" else paste0("Min: ", min, " - "),
          if (is.na(max)) "" else paste0("Max: ", max)
        )
  ...
  if (stnStds) {
    tmp <- DBI::dbGetQuery(
      con,
      paste0(
        "SELECT StnId, StnStd AS StdCode FROM eqstns WHERE StnId IN (",
        paste0(samps_locs$StnId, collapse = ", "),
        ") AND StnStd IS NOT NULL;"
      )
    )
    if (nrow(tmp) == 0) {
      stnStds <- FALSE
      ...
      tmp <- merge(
        ...
        DBI::dbGetQuery(
          ...
          paste0(
            "SELECT StdId, StdCode, StdFlag, StdDesc FROM eqstds WHERE StdCode IN ('",
            paste0(tmp$StdCode, collapse = "', '"),
            "')"
          )
        )
      )
      station_stdVals <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT StdId, ParamId, MaxVal, MinVal FROM eqstdval WHERE StdId IN (",
          paste(tmp$StdId, collapse = ", "),
          ") AND ParamId IN (",
          paste0(params$ParamId, collapse = ", "),
          ");"
        )
      )
      if (nrow(station_stdVals) > 0) {
        # Otherwise there are no station-specific standards for the parameters in the report
  ...
  if (!is.null(SD_exceed)) {
    ...
    SD_vals <- list()
    for (i in unique(samps_locs$StnId)) {
      # There may be duplicate stations in samps_locs, so we need to loop through unique stations. This is done for every parameter in ParamIds
      query <- paste0(
        "SELECT SampleId FROM eqsampls WHERE StnId = ",
        i,
        " AND SampleClass <> 'D'"
      )
      if (!is.null(SD_start)) {
      ...
      if (!is.null(SD_doy)) {
        query <- paste0(
          query,
          " AND DatePart('y', CollectDateTime) IN (",
          paste0(SD_doy, collapse = ", "),
          ")"
        )
      }
      sample_ids <- DBI::dbGetQuery(con, query)

      if (nrow(sample_ids) == 0) {
        next
      }
      ...
      results <- DBI::dbGetQuery(
        con,
        paste0(
          "SELECT ParamId, Result FROM eqdetail WHERE SampleId IN (",
          paste0(sample_ids$SampleId, collapse = ", "),
          ") AND ParamId IN (",
          paste0(ParamIds, collapse = ", "),
          ");"
        )
        ...
        if (length(vals) > 1) {
          ...
          sd_values <- rbind(
            ...
            data.frame(
              ...
              Mean = mean_val,
              SD = sd_val,
              SD_exceed_max = mean_val + (SD_exceed * sd_val),
              SD_exceed_min = mean_val - (SD_exceed * sd_val)
            )
          )
        }
      }
  ...
  top <- data.frame(
    tmp = c(
      paste0(
        "WQ report for EQWin stns  ",
        paste(locations$StnName, collapse = ", ")
      ),
      paste0(
        "For ",
        date,
        if (date_approx > 0) {
          paste0(
            ". Requested stns w/o smpls on this date may show smpls within ",
            date_approx,
            " day(s)."
          )
        } else {
    ...
    tmp7 = NA,
    tmp8 = c(
      paste0("Issued at ", substr(time, 1, 16), " MST"),
      paste0(
        "Created with R package YGwater ",
  ...
  exceedStyle <- openxlsx::createStyle(
    fontColour = "black",
  ...
  if (stnStds) {
    string <- paste0(
      string,
      if (!is.null(stds)) ", " else "",
      "station-specific standards"
    )
  }
  if (!is.null(SD_exceed)) {
    string <- paste0(
      string,
      if (!is.null(stds) | stnStds) ", " else "",
      "SD exceedances above/below ",
      SD_exceed,
      " SD from mean (from",
      if (!is.null(SD_start)) SD_start else " start of records to ",
      if (!is.null(SD_end)) SD_end else "end of records and",
      if (!is.null(SD_doy)) {
        paste0(" on days of year ", paste0(min(SD_doy), " to ", max(SD_doy)))
      } else {
        " for all days of year"
      },
      ")."
    )
  }
  ...
  openxlsx::writeData(
    wb,
    "Report",
    "Stations (sample date-time)",
    startCol = 2 + nrow(standards),
    startRow = 4,
    colNames = FALSE
  )
  openxlsx::writeData(
  ...
  openxlsx::addStyle(
    ...
    cols = c(
      (2 + nrow(standards)):(2 +
        nrow(standards) +
        (ncol(final_table) - (nrow(standards) + 2)))
    )
  )
  openxlsx::addStyle(
    wb,
  ...
  for (i in names(final_table)[(2 + nrow(standards)):ncol(final_table)]) {
    ...
    if (stnStds) {
      ...
      stn_std_comment <- station_stdVals[
        station_stdVals$StnId == id,
        c("ParamName", "string")
      ]
      if (nrow(stn_std_comment) > 0) {
        ...
        comment <- paste(
          ...
          paste0(
            "Station-Specific Standards (",
            unique(station_stdVals[station_stdVals$StnId == id, "StdDesc"]),
            ") "
          ),
          df_string,
          sep = "\n"
    ...
    openxlsx::writeComment(
      ...
      "Report",
      row = 5,
      col = which(names(final_table) == i),
      comment = openxlsx::createComment(
        comment,
        visible = FALSE,
        height = if (nrow(stn_std_comment) > 0) 20 else 1
      )
    )
  }
  ...
  exceed_comments <- matrix(
    character(0),
    nrow = nrow(final_table),
    ncol = ncol(final_table) - (1 + nrow(standards))
  )
  if (!is.null(stds)) {
    for (i in 1:nrow(final_table)) {
      ...
      for (j in 1:nrow(standards)) {
        std_applies <- stdVals[
          stdVals$ParamName == param & stdVals$StdId == standards[j, "StdId"],
        ]
        if (
          nrow(std_applies) == 0 ||
            (is.na(std_applies$MaxVal) & is.na(std_applies$MinVal))
        ) {
          # If there is no standard for this parameter, skip
          next()
        } else {
          # There is a standard, so check if the MaxVal and the MinVal standards are exceeded
          for (k in (2 + nrow(standards)):ncol(final_table)) {
            # Now we can work left to right cell by cell
            if (is.na(final_table[i, k])) {
              next
            } # it's character at this point, and NA means that there is nothing in the field
            compare_value <- suppressWarnings(as.numeric(final_table[i, k])) # The value to compare against the standard
            if (is.na(compare_value)) {
              next
            } # If the value is NA, skip. This happens notably when the value is < the detection limit.
            for (l in c("MinVal", "MaxVal")) {
              if (is.na(std_applies[[l]])) {
                next
              } # If the standard is NA, skip
              ...
              if (grepl("=", std_applies[[l]])) {
                ...
                calc_id <- DBI::dbGetQuery(
                  con,
                  paste0(
                    "SELECT CalcId FROM eqcalcs WHERE CalcCode = '",
                    sub("=", "", std_applies[[l]]),
                    "';"
                  )
                )$CalcId
                ...
                # It's a simple standard! Nice and easy.
                min_max <- as.numeric(std_applies[[l]])
              }
              if (l == "MinVal") {
                if (compare_value < min_max) {
                  exceed_comments[i, (k - (1 + nrow(standards)))] <- paste0(
                    if (
                      !is.na(exceed_comments[i, (k - (1 + nrow(standards)))])
                    ) {
                      exceed_comments[i, (k - (1 + nrow(standards)))]
                    },
                    if (
                      !is.na(exceed_comments[i, (k - (1 + nrow(standards)))])
                    ) {
  ...
  if (stnStds) {
    # Finally, check the station-specific standard. If any are exceeded add to the matrix.
    row_names <- names(final_table)[(2 + nrow(standards)):ncol(final_table)]
    for (i in 1:length(row_names)) {
      name <- row_names[i]
      col_number_final <- which(names(final_table) == name)
      name <- sub(" \\([0-9]+\\)$", "", name)
      id <- unique(samps_locs[
        paste0(samps_locs$StnName, " (", samps_locs$CollectDateTime, ")") ==
          name,
        "StnId"
      ]) # station ID
      ...
      if (nrow(stn_std) > 0) {
        # Then a standard of some sort applies
        for (j in 1:nrow(final_table)) {
          # Now we can go cell by cell
          ...
          if (
            ...
            # There is a standard, so check if the MaxVal and the MinVal standards are exceeded
            if (is.na(final_table[i, col_number_final])) {
              next
            } # it's character at this point, and NA means that there is nothing in the field

            compare_value <- suppressWarnings(as.numeric(final_table[
              j,
              col_number_final
            ])) # The value to compare against the standard
            if (is.na(compare_value)) {
              next
            } # If the value is NA, skip. This happens notably when the value is < the detection limit.
            ...
            for (l in c("MinVal", "MaxVal")) {
              ...
              if (l == "MinVal") {
                if (compare_value < min_max) {
                  exceed_comments[j, i] <- paste0(
                    if (!is.na(exceed_comments[j, i])) exceed_comments[j, i],
                    if (!is.na(exceed_comments[j, i])) "\n",
                    "Below ",
                    std_applies$StdCode,
                    " of ",
                    min_max
  ...
  if (!is.null(SD_exceed)) {
    ...
    for (i in 1:length(row_names)) {
      ...
      if (is.null(sd_vals)) {
        next
      } # If there are no SD values for this station, skip

      if (nrow(sd_vals) > 0) {
        # Then sd values were calculated for the station
        for (j in 1:nrow(final_table)) {
          # Now we can go cell by cell on final_table
          param <- final_table$Parameter[j]
          # see if there's a corresponding entry in sd_vals
          sd_applies <- sd_vals[sd_vals$ParamCode == param, ]
          if (nrow(sd_applies) == 0) {
            # If there is no SD for this parameter, skip
            next
          } else {
            # There is a SD value calculated. Check if the max and min values are exceeded
            if (is.na(final_table[i, col_number_final])) {
              next
            } # it's character at this point, and NA means that there is nothing in the field
            ...
            compare_value <- suppressWarnings(as.numeric(final_table[
              j,
              col_number_final
            ])) # The value to compare agains the standard
            if (is.na(compare_value)) {
              next
            } # If the value is NA, skip. This happens notably when the value is < the detection limit.

            for (l in c("SD_exceed_max", "SD_exceed_min")) {
              if (is.na(sd_applies[[l]])) {
                next
              } # If the SD is NA, skip
              ...
              if (l == "SD_exceed_min") {
                if (compare_value < min_max) {
                  exceed_comments[j, i] <- paste0(
                    ...
                    "Below ",
                    SD_exceed,
                    " standard deviations from the historical mean of ",
                    round(sd_applies$Mean, 6),
                    "."
                  )
                }
  ...
  for (i in 1:nrow(exceed_comments)) {
    for (j in 1:ncol(exceed_comments)) {
      ...
      openxlsx::writeComment(
        ...
        "Report",
        row = 5 + i,
        col = j + (1 + nrow(standards)),
        comment = openxlsx::createComment(comment, visible = FALSE, height = 20)
      )
      openxlsx::addStyle(
        wb,
        "Report",
  ...
  openxlsx::freezePane(
    ...
    "Report",
    firstActiveRow = 6,
    firstActiveCol = (2 + nrow(standards))
  )
