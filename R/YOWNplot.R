...
YOWNplot <- function(
  ...
  if (!is.null(saveTo)) {
    if (tolower(saveTo) == "desktop") {
      ...
      if (!format %in% c("png", "pdf")) {
        stop(
          "Specify format as 'png' or 'pdf', or set 'saveTo' to NULL to only get a console output."
        )
      }
    } else if (saveTo == "choose") {
      # Select save path using GUI
      saveTo <- rstudioapi::selectDirectory(
        caption = "Select Save Folder",
        path = file.path(Sys.getenv("USERPROFILE"), "Desktop")
      )
    } else if (!dir.exists(saveTo)) {
      stop(
        "Specified directory does not exist. Consider specifying save path as one of 'choose' or 'desktop'; refer to help file."
      )
    }
  }
  ...
  timeseries$grade_description[
    timeseries$grade_description != "A" &
      timeseries$grade_description != "B" &
      timeseries$grade_description != "C" &
      timeseries$grade_description != "E" &
      timeseries$grade_description != "MISSING DATA"
  ] <- "REDACTED"
  ...
  timeseries$value[
    timeseries$grade_description != "A" &
      timeseries$grade_description != "B" &
      timeseries$grade_description != "C" &
      timeseries$grade_description != "D"
  ] <- NA
  ...
  fulldf$lag_val <- difftime(
    fulldf$timestamp_MST,
    fulldf$ts_lag,
    units = "hours"
  ) # format lag as hours
  ...
  if (nrow(gapdf != 0)) {
    ...
    for (i in 1:nrow(gapdf)) {
      df <- data.frame(
        seq.POSIXt(
          from = as.POSIXct(gapdf[i, 1]),
          by = "-1 hour",
          length.out = gapdf[i, "lag_val"]
        ),
        NA,
        as.character(-5),
        "MISSING DATA",
        gapdf$approval_level[i],
        gapdf$approval_description[i],
        NA,
        NA
      )
  ...
  fulldf <- fulldf %>%
    dplyr::mutate(
      "date" = format(fulldf$timestamp_MST, "%Y-%m-%d"), # Add date column (YMD)
      ...
      "monthday" = format(fulldf$timestamp_MST, "%m-%d")
    ) # Add month-day column
  datestats <- suppressWarnings(
    dplyr::group_by(fulldf, date) %>% # Calculate statistics by date (ie. Jan. 1, 2000)
      dplyr::summarize(
        "datemin" = min(.data$value, na.rm = TRUE),
        "datemax" = max(.data$value, na.rm = TRUE),
        "datemean" = mean(.data$value, na.rm = TRUE)
      )
  )
  fulldf <- suppressMessages(dplyr::full_join(fulldf, datestats)) # Join full df to datestats
  daystats <- suppressWarnings(
    dplyr::group_by(fulldf, .data$monthday) %>% # Calculate year-over-year daily statistics (ie. Jan. 1)
      dplyr::summarize(
        "daymin" = min(.data$datemin, na.rm = TRUE),
        "daymax" = max(.data$datemax, na.rm = TRUE),
        daymean = mean(.data$datemean, na.rm = TRUE),
        N = dplyr::n()
      )
  )
  ...
  if (paste(tolower(dateRange), collapse = ",") == "all") {
    dateRange <- c(
      min(stats::na.omit(fulldf$timestamp_MST)),
      max(stats::na.omit(fulldf$timestamp_MST))
    )
  } else if (paste(tolower(dateRange), collapse = ",") == "1yr") {
    dateRange <- c(
      (max(stats::na.omit(fulldf$timestamp_MST)) - lubridate::years(1)),
      max(stats::na.omit(fulldf$timestamp_MST))
    )
  ...
  plotdf <- subset(
    fulldf,
    fulldf$timestamp_MST >= (min(dateRange)) &
      fulldf$timestamp_MST <= (max(dateRange))
  )
  ...
  if (chartXinterval == "auto") {
    diff <- as.numeric(difftime(
      max(plotdf$timestamp),
      min(plotdf$timestamp),
      units = "days"
    ))
  ...
  if (is.numeric(smooth)) {
    plotdf <- plotdf %>%
      dplyr::mutate(
        "datemean" = zoo::rollapply(
          data = plotdf$datemean,
          FUN = mean,
          width = smooth,
          partial = TRUE
        )
  ...
  subtitle <- ggplot2::ggplot() +
    ...
    ggplot2::labs(
      title = paste0(
        ...
        timeSeriesID,
        "\nLatitude: ",
        datalist[["metadata"]][5, 2],
        ", ",
        "Longitude: ",
        datalist[["metadata"]][6, 2],
        ", ",
        "Elevation: ",
        datalist[["metadata"]][7, 2],
        " ",
        datalist[["metadata"]][8, 2]
      )
    ) +
    ggplot2::theme(
      plot.title = ggplot2::element_text(
  ...
  if (
    tolower(stats) == "ribbon" &
      (max(fulldf$timestamp_MST, na.rm = TRUE) -
        min(fulldf$timestamp_MST, na.rm = TRUE) <
        730)
  ) {
    warning(
      "Insufficient data for ribbon stats calculation, plot produced with no stats instead"
    )
    stats <- FALSE
  }
  ...
  if (tolower(stats) == "ribbon") {
    ...
    plot <- ggplot2::ggplot() +
      ggplot2::geom_ribbon(
        ...
        ggplot2::aes(
          ymin = .data$daymin,
          ymax = .data$daymax,
          x = .data$timestamp_MST,
          fill = "Range of Historical Max & Min Daily Groundwater Levels"
        )
      ) +
      ggplot2::scale_fill_manual(
        name = "",
        values = c(
          "Range of Historical Max & Min Daily Groundwater Levels" = "#B8BDC3"
        )
      ) +
      ggplot2::geom_line(
        data = plotdf,
        ggplot2::aes(
          x = .data$timestamp_MST,
          y = .data$daymean,
          colour = "Historical Mean Daily Groundwater Level"
        ),
        linewidth = 0.3,
        na.rm = TRUE
      ) +
      ggplot2::scale_colour_manual(
        name = "",
        values = c("Historical Mean Daily Groundwater Level" = "#0097A9")
      ) +
      ...
      ggplot2::geom_line(
        ...
        ggplot2::aes(
          x = .data$timestamp_MST,
          y = .data$datemean,
          colour = "Daily Average Groundwater Level"
        ),
        linewidth = 0.5,
        na.rm = TRUE
      ) +
      ggplot2::scale_colour_manual(
        name = "",
        values = c("Daily Average Groundwater Level" = "#244C5A")
      ) +
      ggplot2::guides(
    ...
    title <- title +
      ggplot2::labs(
        title = paste0(
          "Groundwater Level Statistics Chart: ",
    ...
    caption <- caption +
      ggplot2::labs(
        title = paste0(
          "Max & Min data calculated from period of record from ",
          strftime(
            as.POSIXct(min(stats::na.omit(timeseries$timestamp_MST))),
            format = "%Y-%m-%d"
          ),
          " to ",
          strftime(
            as.POSIXct(max(stats::na.omit(timeseries$timestamp_MST))),
            format = "%Y-%m-%d"
          ),
    ...
    plot <- ggplot2::ggplot() +
      ...
      ggplot2::scale_colour_manual(
        name = "",
        values = c("Water Level (title year)" = "#244C5A")
      ) +
      ggplot2::guides(
    ...
    if (stats != FALSE) {
      plot <- plot +
        ggplot2::scale_y_reverse(
          name = ytitle,
          limits = c(
            round_any(max(stats::na.omit(plotdf$daymax)), 0.5, f = ceiling),
        ...
        plot <- plot +
          ...
          ggplot2::geom_path(
            ...
            ggplot2::aes(
              x = .data$timestamp_MST,
              y = round_any(
                max(stats::na.omit(.data$daymax)),
                0.5,
                f = ceiling
              ),
              colour = factor(.data$grade_description),
              group = 1
            ),
            linewidth = 2.5,
            show.legend = FALSE
        ...
        plot <- plot +
          ...
          ggplot2::geom_path(
            ...
            ggplot2::aes(
              x = .data$monthday,
              y = round_any(
                max(stats::na.omit(plotdf$datemax)),
                0.5,
                f = ceiling
              ),
              colour = factor(.data$grade_description),
      ...
      plot <- plot +
        ...
        ggplot2::scale_y_reverse(
          name = ytitle,
          limits = c(
            round_any(max(stats::na.omit(plotdf$value)), 0.5, f = ceiling),
    ...
    if (stats != FALSE) {
      plot <- plot +
        ...
        ggplot2::geom_path(
          ...
          ggplot2::aes(
            x = .data$timestamp_MST,
            y = round_any(min(stats::na.omit(.data$daymin)), 0.25, f = floor),
            colour = factor(.data$grade_description),
            group = 1
          ),
          linewidth = 2.5,
        ...
        ggplot2::scale_y_continuous(
          name = ytitle,
          limits = c(
            round_any(min(stats::na.omit(plotdf$daymin)), 0.25, f = floor),
            round_any(max(stats::na.omit(plotdf$daymax)), 0.5, f = ceiling)
          ),
          breaks = seq(
            floor(min(stats::na.omit(plotdf$daymin))),
            ceiling(max(stats::na.omit(plotdf$daymax))),
            by = 0.25
          ),
          expand = c(0, 0)
        )
      ...
      plot <- plot +
        ...
        ggplot2::geom_path(
          ...
          ggplot2::aes(
            x = .data$timestamp_MST,
            y = round_any(min(stats::na.omit(.data$daymean)), 0.25, f = floor),
            colour = factor(.data$grade_description),
            group = 1
          ),
          linewidth = 2.5,
        ...
        ggplot2::scale_y_continuous(
          name = ytitle,
          limits = c(
            round_any(min(stats::na.omit(plotdf$daymean)), 0.25, f = floor),
            round_any(max(stats::na.omit(plotdf$daymean)), 0.5, f = ceiling)
          ),
          breaks = seq(
            floor(min(stats::na.omit(plotdf$daymean))),
            ceiling(max(stats::na.omit(plotdf$daymean))),
            by = 0.25
          ),
          expand = c(0, 0)
        )
