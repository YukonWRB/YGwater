...
YOWNplot <- function(
    ...
    if (stats != FALSE) {
      plot <- plot +
        ggplot2::scale_y_reverse(
          ...
          limits = c(
            plyr::round_any(
              max(stats::na.omit(plotdf$daymax)),
              0.5,
              f = ceiling
            ),
      ...
      plot <- plot +
        ...
        ggplot2::scale_y_reverse(
          ...
          limits = c(
            plyr::round_any(
              max(stats::na.omit(plotdf$value)),
              0.5,
              f = ceiling
            ),
    ...
    if (stats != FALSE) {
      plot <- plot +
        ...
        ggplot2::scale_y_continuous(
          ...
          limits = c(
            plyr::round_any(
              min(stats::na.omit(plotdf$daymin)),
              0.25,
              f = floor
            ),
            plyr::round_any(
      ...
      plot <- plot +
        ...
        ggplot2::geom_path(
          ...
          ggplot2::aes(
            x = .data$timestamp_MST,
            y = plyr::round_any(
              min(stats::na.omit(.data$daymean)),
              0.25,
              f = floor
            ),
            colour = factor(.data$grade_description),
        ...
        ggplot2::scale_y_continuous(
          ...
          limits = c(
            plyr::round_any(
              min(stats::na.omit(plotdf$daymean)),
              0.25,
              f = floor
            ),
            plyr::round_any(
              max(stats::na.omit(plotdf$daymean)),
              0.5,
              f = ceiling
            )
          ),
  ...
  final_plot <- cowplot::ggdraw() +
    cowplot::draw_image(
      "G:\\water\\Groundwater\\2_YUKON_OBSERVATION_WELL_NETWORK\\4_YOWN_DATA_ANALYSIS\\1_WATER_LEVEL\\00_AUTOMATED_REPORTING\\01_MARKUP_IMAGES\\Template_grades.jpg"
    ) +
