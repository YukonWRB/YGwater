...
YGimgupload <- function(host = getOption("shiny.host", "127.0.0.1"), port = getOption("shiny.port"), dbName = "aquacache", dbHost = Sys.getenv("aquacacheHost"), dbPort = Sys.getenv("aquacachePort"), dbUser = Sys.getenv("aquacacheUser"), dbPass = Sys.getenv("aquacachePass"), accessPath = "//env-fs/env-data/corp/water/Data/Databases_virtual_machines/databases/EQWinDB", server = FALSE, public = TRUE){
  ...
  rlang::check_installed("dplyr", reason = "required to use YGwater app")
  ...
  appDir <- system.file("apps/YGimgupload", package = "YGwater")
  
  if (appDir == "") {
    stop("YGimgupload app not found.")
  }
  ...
  if (!DBI::dbExistsTable(con, "images")) {
