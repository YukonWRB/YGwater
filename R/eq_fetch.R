...
eq_fetch <- function(
  ...
  eqsampls <- DBI::dbGetQuery(
    EQWin,
    "SELECT SampleId, StnId, CollectDateTime FROM eqsampls"
  ) %>%
  ...
  eqparams <- as.data.frame(DBI::dbGetQuery(
    EQWin,
    "SELECT ParamId, ParamCode, Units FROM eqparams"
  ))
  tryCatch(
    {
    ...
    error = function(e) {
      message(paste(
        "The following parameters do not match exactly what is in EQWin:",
        paste(setdiff(paramIDs, params$ParamCode), collapse = ", ")
      ))
    }
  )
  ...
  results <- DBI::dbGetQuery(
    ...
    paste0(
      "SELECT ",
      paste0('SampleId', ", ", 'ParamId', ", ", 'Result'),
      " FROM eqdetail WHERE ParamID IN (",
      paste(eqparams$ParamId, collapse = ", "),
      ") AND SampleId IN (",
      paste0(eqsampls$SampleId, collapse = ", "),
      ")"
    )
  )
  ...
  if (BD == 0) {
    ...
    results$Result[isBD] <- round(
      as.numeric(gsub("<(.*)", "\\1", results$Result[isBD])) / 2,
      digits = 7
    )
    rm(isBD)
  } else if (BD == 3) {
    isBD <- grepl("<", results$Result)
    results$Result[isBD] <- round(
      as.numeric(gsub("<(.*)", "\\1", results$Result[isBD])) / sqrt(2),
      digits = 7
    )
    rm(isBD)
  }
  ...
  suppressMessages(
    sampledata <- merge3 %>%
      dplyr::filter(.data$StnCode %in% stationIDs) %>%
      dplyr::mutate(
        Param = paste0(merge3$ParamCode, " (", merge3$Units, ")")
      ) %>%
      dplyr::select(
        .data$StnCode,
        .data$CollectDateTime,
        .data$StnType,
        .data$Param,
        .data$Units,
        .data$Result
      ...
      dplyr::summarize(
        Result = suppressWarnings(mean(as.numeric(.data$Result)))
      ) %>%
      tidyr::pivot_wider(
        id_cols = c("StnCode", "CollectDateTime", "StnType"),
  ...
  if (apply_standards) {
    ...
    eqstds <- as.data.frame(
      DBI::dbReadTable(EQWin, "eqstds") %>%
        subset(select = c("StdId", "StdCode", "StdName", "udf_StnGroup"))
    )
    eqstdval <- as.data.frame(
      DBI::dbReadTable(EQWin, "eqstdval") %>%
    ...
    stds <- dplyr::filter(
      ...
      stds$StdCode %in%
        utils::select.list(
          choices = sort(unique(stds$StdCode)),
    ...
    stds <- stds[, c(
      ...
      "MinVal",
      "Units"
    )] # Select relevant columns, reorder
    ...
    for (i in unique(sampledata$StnCode)) {
      ...
      std_calcs <- eq_std_calc(
        sampledata = sampledatafilt,
        calcs = std_calc_tmp
      )
      ...
      stnstd <- tidyr::pivot_wider(
        stnstd,
        id_cols = c("StdName", "StdCode"),
        names_from = .data$Param,
        values_from = .data$MaxVal
      )
      ...
      head_data <- colnames(sampledata)[
        -c(grep("\\(*.\\)", colnames(sampledata)))
      ]
      ...
      stnstd <- stnstd %>% # Remove std params that do not have exist in sampledata
        dplyr::select(c(
          "StdName",
          "StdCode",
          params_std[params_std %in% params_data]
        ))
  ...
  for (i in unique(sampledata$StnCode)) {
    ...
    stndata <- sampledata %>%
      ...
      dplyr::select(c(
        ...
        "StnType",
        which(
          as.vector(sapply(names(sampledata), function(x) {
            sub(" \\(.*", "", x)
          })) %in%
            paramIDs
        )
    ...
    if (apply_standards) {
      list[["stnstd"]] <- stdlist[[i]] %>%
        dplyr::select(c(
          ...
          "StdCode",
          which(
            as.vector(sapply(names(stnstd), function(x) {
              sub(" \\(.*", "", x)
            })) %in%
              paramIDs
          )
