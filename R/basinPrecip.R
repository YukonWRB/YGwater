...
#' @param maptype Choose from 'leaflet' or 'static'. Leaflet maps are interactive and return the entire Yukon territory with the requested basin highlighted. Static maps are printed to the console and show only the requested basin. Only used if map = TRUE.
...
)
{
  ...
  if (!maptype %in% c("leaflet", "static")) {
    stop("Parameter `maptype` should be either 'leaflet' or 'static'.")
  }
  ...
  if (grepl("^[0-9]{2}[\\.]{1}[0-9]*[ ]{1}[-]{1}[0-9]*[\\.]{1}[0-9]*", location)) {
    tryCatch({
      ...
      location <- terra::vect(location, geom = c("long", "lat"), crs = "+proj=longlat +EPSG:3347")
      type <- "longlat"
    }, error = function(e) {
      stop("Please check your input for `location`. It looks like you're trying to input decimal degrees, so please ensure that the latitude and longitude are only numbers separated by a comma.")
    })
  } else if (nrow(DBI::dbGetQuery(con, paste0("SELECT location_id FROM locations WHERE location = '", location, "';"))) == 1) {
    type <- "DB"
  ...
  tryCatch({
    if (type == "longlat") {
      ...
      basins <- getVector(layer_name = "Drainage basins", silent = TRUE, con = con)
      ...
      basin <- getVector(layer_name = "Drainage basins", feature_name = requested_point, con = con, silent = TRUE)
  ...
  if (map) {
    tryCatch({
      ...
      if (maptype == "leaflet") {
        leafmap <- terra::plet(total, col = c("#F0F7FF", raster_col), tiles = NULL) |>
          terra::lines(basin, col = "darkred")
        if (type == "longlat") {
          leafmap <- leafmap |> terra::points(location, pch = 17, col = "darkorchid1", cex = 2)
        }
        leafmap <- leafmap |>
          terra::lines(roads, lwd = 1.5, alpha = 0.7, dashArray = "4 1 2 3")
        ...
        #   leafmap <- leafmap |>
        #     terra::lines(streams, lwd = 0.08, col = "blue", alpha = 1, )
        ...
        leafmap <- leafmap |>
          terra::polys(waterbodies, col = "blue", border = "blue", alpha = 0.5, lwd = 0.001) |>
          terra::points(communities, cex = 1.5) |>
          terra::lines(borders, lwd = 2, dashArray = "4") 
        
        if (title) {
          text <- paste0("Precipitation as mm of water equivalent from ", actual_times[1], " to ", actual_times[2], " UTC  <br> Watershed: ", basin$feature_name, ", ", stringr::str_to_title(basin$description))
          leafmap <- leafmap |>
            leaflet::addControl(text, position = "topright", className = "map-title")
        }
        plot <- leafmap
      } else {
        ...
        terra::plot(cropped_precip_rast, col = c("#F0F7FF", raster_col), buffer = TRUE, alpha = 0.5)
        ...
        terra::text(communities, labels = communities$feature_name, pos = 4, offset = 1, font = 2, cex = 0.9)
        if (type == "longlat") {
          terra::text(location, labels = paste0(requested_point[1], ", ", requested_point[2]), col = "black", pos = 4, offset = 1, font = 2)
        }
        if (title) {
          text <- paste0("Precipitation as mm of water equivalent from ", actual_times[1], " to ", actual_times[2], " UTC  \nWatershed: ", basin$feature_name, ", ", stringr::str_to_title(basin$description))
