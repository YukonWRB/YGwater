...
EQ_crosstab <- function(folder = "choose", save_path = NULL) {
  if (folder == "choose") {
    ...
    folder <- rstudioapi::selectDirectory(
      caption = "Select Lab Report Folder",
  ...
  tryCatch(
    {
      for (i in 1:length(files)) {
        header[[i]] <- data.table::as.data.table(openxlsx::read.xlsx(
          files[i],
          sheet = 1,
          startRow = 1,
          rows = c(1:14),
        ...
        data[[i]] <- data.table::as.data.table(openxlsx::read.xlsx(
          files[i],
          sheet = 1,
          startRow = 15
        ))
    ...
    error = function(e) {
      stop(
        "There was an error reading the files. Make sure that none are open in Excel!"
      )
    }
  )
  ...
  for (i in 1:length(data)) {
    if (i == 1) {
      ...
      if (nrow(results[results$Parameter.Code == "P-T", ]) > 1) {
        results[results$Parameter.Code == "P-T", ][
          1,
          "Parameter.Code"
        ] <- "P-PO4T"
      }
      ...
      data.table::setnames(
        results,
        c(names(results)[1:3], as.character(tmp.head[2, 2:ncol(tmp.head)]))
      )
      # Deal with headers
      head <- header[[i]]
      data.table::setnames(
        head,
        c("Variable", as.character(header[[i]][2, 2:ncol(header[[i]])]))
      )
    } else {
      # Deal with data
      tmp <- data[[i]]
      ...
      data.table::setnames(
        tmp,
        c(names(tmp)[1:3], as.character(tmp.head[2, 2:ncol(tmp.head)]))
      )
      ...
      data.table::setnames(
        tmp.head,
        as.character(c(tmp.head[2, 1:ncol(tmp.head)]))
      )
  ...
  openxlsx::saveWorkbook(
    wb,
    paste0(save_path, "/EQ_crosstab_concat_", Sys.Date(), ".xlsx"),
    overwrite = TRUE
  )
