...
DB_get_meta <- function(con = hydrometConnect(), location = "all", save_path = NULL) {
  ...
  if (location == "all"){
    location <- DBI::dbGetQuery(con, "SELECT location FROM locations")[,1]
  }

  meta <- data.frame()
  datum <- data.frame()
  for (i in location){
    latlong <- DBI::dbGetQuery(con, paste0("SELECT latitude, longitude FROM locations WHERE location = '", i, "'"))
    datums <- DBI::dbGetQuery(con, paste0("SELECT datum_id_from, datum_id_to, conversion_m, current FROM datum_conversions WHERE location = '", i, "'"))
    datum_codes <- unique(c(datums$datum_id_from, datums$datum_id_to))
    for (j in 1:nrow(datums)){
      datums$datum_id_from[j] <- DBI::dbGetQuery(con, paste0("SELECT datum_name_en FROM datum_list WHERE datum_id = '", datums$datum_id_from[j], "'"))
      datums$datum_id_to[j] <- DBI::dbGetQuery(con, paste0("SELECT datum_name_en FROM datum_list WHERE datum_id = '", datums$datum_id_to[j], "'"))
    }
    datums$location <- i
    datums <- datums[, c(5, 1,2,3,4)]
    names(datums) <- c("location", "origin datum", "end datum", "conversion (m)", "current")

    name <- DBI::dbGetQuery(con, paste0("SELECT name FROM locations WHERE location = '", i, "'"))
