...
#'
#' @return Moves YOWN html file to backups folder and appropriate YOWN Active Wells folder. Uploads data to Aquarius if `aq_upload = TRUE` after performing unit checks and conversions. Adds pressure column if missing, based on depth column and conversions calcs on In Situ website.
#' @export

html_processing <- function(
  file,
  aq_upload = TRUE,
  ...
  convert_pressure <- function(column, pressure_unit) {
    ...
    if (pressure_unit == "m") {
      ...
      conversion_factor <- pressure_conversion_factors$to_m_H2O[
        pressure_conversion_factors$unit == pressure_unit
      ]
  ...
  convert_depth <- function(column, depth_unit) {
    ...
    if (depth_unit != "m") {
      # Find the conversion factor for the specified unit
      conversion_factor <- depth_conversion_factors$to_m[
        depth_conversion_factors$unit == depth_unit
      ]
      # If the unit is found, apply the conversion; if not, throw an error and stop code
      if (length(conversion_factor) == 1) {
        column <- column * conversion_factor
  ...
  parse_properties <- function(html, section_name) {
    html %>%
      rvest::html_nodes(
        xpath = sprintf(
          "//tr[@class='sectionMember'][td[@isi-group-member='%s']]",
          section_name
        )
      ) %>%
      rvest::html_text() %>%
      stringr::str_split(" = ") %>%
      purrr::map_df(
        ~ data.frame(Property = .x[1], Value = .x[2], stringsAsFactors = FALSE)
      )
  }
  ...
  df <- as.data.frame(
    matrix(data_rows, ncol = n_cols, byrow = TRUE),
    stringsAsFactors = FALSE
  ) %>%
    dplyr::mutate(
      V1 = lubridate::as_datetime(.data$V1, tz = "MST") %>%
  ...
  if ("Pressure" %in% colnames(final_data)) {
    final_data$Pressure <- convert_pressure(
      column = final_data$Pressure,
      pressure_unit
    )
    pressure_unit <- "m"
  }
  ...
  for (i in c("Wlevel_Hgt.level_RAW", "Water Temp.TEMPERATURE")) {
    if (i == "Wlevel_Hgt.level_RAW") {
      # Upload level data
      temp <- data.frame(
        Time = final_data$Date,
        Value = final_data$`Pressure (m)`
      )
      tryCatch(
        {
          start <- Sys.time()
  ...
  new_data <- c(
    well_loc,
    "In Situ",
    instrument_properties$Value[
      instrument_properties$Property == "Device Model"
    ],
    NA,
  ...
  tryCatch(
    ...
    warning = function(w) {
      write(
        "Logger tracking FAILED, make sure nobody has the logger tracking sheet open. Re-add HTML file to logger dropbox to complete tracking.",
        file = paste0(dropbox, "/LOGBOOK.txt"),
        append = TRUE,
        sep = "\n"
      )
  ...
  suppressWarnings(dir.create(paste0(
    "//env-fs/env-data/corp/water/Groundwater/2_YUKON_OBSERVATION_WELL_NETWORK/1_YOWN_SITES/1_ACTIVE_WELLS/",
    dir,
    ...
    tryCatch(
      {
        ...
        write(
          "The html file was successfully moved to its final resting places",
          file = paste0(dropbox, "/LOGBOOK.txt"),
          append = TRUE,
          sep = "\n"
        )
