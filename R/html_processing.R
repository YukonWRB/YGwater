  ...
  if (aq_upload) {
    for (i in c("Wlevel_Hgt.level_RAW", "Water Temp.TEMPERATURE", "Conductivity Field.Econdy-F")) {
      if (i == "Wlevel_Hgt.level_RAW") { # Upload level data
        temp <- data.frame(Time = final_data$Date, Value = final_data$`Level (m)`)
        tryCatch({
          start <- Sys.time()
        ...
        tryCatch({
          temp <- data.frame(Time = final_data$Date, Value = final_data$'Conductivity (\u00B5S/cm)')
          start <- Sys.time()
          result <- YGwater::aq_upload(well_loc, i, temp)
          end <- Sys.time() - start
