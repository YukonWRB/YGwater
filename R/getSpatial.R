...
getRaster <- function(
  ...
  # Check if the query with clauses returns any raster data
  if (!is.null(clauses)) {
    test_query <- paste0(
      "SELECT COUNT(*) as count FROM ",
      paste(DBI::dbQuoteIdentifier(con, tbl_name), collapse = "."),
      " ",
      clauses,
      " AND ",
    ...
    raster_count <- DBI::dbGetQuery(con, test_query)$count
    if (raster_count == 0) {
      cli::cli_abort("No raster data found matching the specified clauses.")
    }
  }
  ...
  dbTableNameFix <- function(conn = NULL, t.nm, as.identifier = TRUE) {
    ...
    if (
      length(t.nm) == 1 &&
        !is.null(conn) &&
        !inherits(conn, what = "AnsiConnection")
    ) {
      schemalist <- DBI::dbGetQuery(
        conn,
        "select nspname as s from pg_catalog.pg_namespace;"
      )$s
      ...
      schema <- gsub(
        " ",
        "",
        unlist(strsplit(schema, ",", fixed = TRUE)),
        fixed = TRUE
      )
    ...
    if (length(t.nm) > 2) {
      cli::cli_abort(
        "Invalid PostgreSQL table/view name. Must be provided as one ('table') or two-length c('schema','table') character vector."
      )
    }
    if (is.null(conn)) {
      conn <- DBI::ANSI()
  ...
  if (is.null(tab.list)) {
    ...
    cli::cli_abort(
      "Table '{namechar}' raster column '{col_name}' not found. Available raster columns: {paste(tab.list, collapse = ', ')}"
    )
  }
  tmp.query <- paste0(
    "SELECT st_numbands(",
  ...
  if (isTRUE(bands)) {
    ...
    cli::cli_abort(
      "Selected band(s) do not exist in PostGIS raster: choose band numbers between {min(nbs)} and {max(nbs)}."
    )
  }
  ...
  tmp.query <- paste0(
    ...
    rastque,
    " IS NOT NULL",
    if (!is.null(clauses2)) paste0(" ", clauses2) else "",
    ";"
  )
  ...
  if (is.na(p4s)) {
    cli::cli_alert_warning(
      "Table SRID not found. Projection will be undefined (NA)"
    )
  }
  tmp.query <- paste0(
  ...
  if (!al) {
    tmp.query <- paste0(
      "SELECT min(ST_UpperLeftX(",
      rastque,
      ")) ux, max(ST_UpperLeftY(",
      rastque,
      ")) uy FROM ",
      nameque,
      ";"
  ...
  if (is.null(boundary)) {
    ...
    if (inherits(boundary, "sf")) {
      ...
      boundary <- c(
        terra::ext(boundary)[4],
        terra::ext(boundary)[3],
        terra::ext(boundary)[2],
        terra::ext(boundary)[1]
      )
    }
...
getVector <- function(
  ...
  if (
    is.null(geom_id) &
      is.null(layer_name) &
      is.null(feature_name) &
      is.null(geom_type)
  ) {
    stop(
      "You need to specify at least one of the geom_id, layer_name, feature_name, or geom_type."
    )
  }
  if (!is.null(geom_type)) {
    if (length(geom_type) > 1) {
      ...
      stop(
        "Parameter geom_type is not one of the possible choices. Refer to the help file."
      )
    }
  }
  ...
  query <- paste0(
    "SELECT geom_id, layer_name, feature_name, geom_type FROM ",
    schema,
    ".",
    table,
    " WHERE"
  ...
  if (length(unique(tbl$geom_type)) > 1) {
    if (!silent) {
      message(
        "Your query resulted in more than one geometry type: ",
        paste(unique(tbl$geom_type), collapse = ", AND "),
        " were returned. This may result in a non-functional terra object if, for example, lines and polygons are returned together."
      )
    }
  }
  ...
  if (silent) {
    res <- suppressMessages(rpostgis::pgGetGeom(
      con,
      name = c(schema, table),
      geom = "geom",
      gid = "geom_id",
