...
getRaster <- function (conn, name = c("spatial", "rasters"), rast = "rast", bands = 1, boundary = NULL, 
    clauses = NULL, progress = TRUE) 
{
    ...
    namechar <- gsub("'", "''", paste(gsub("^\"|\"$", "", name1), 
        collapse = "."))
    rastque <- DBI::dbQuoteIdentifier(conn, rast)
    ...
    tmp.query <- paste0("SELECT r_raster_column AS geo FROM raster_columns\n  WHERE (r_table_schema||'.'||r_table_name) = '", 
        namechar, "';")
    tab.list <- DBI::dbGetQuery(conn, tmp.query)$geo
    if (is.null(tab.list)) {
    ...
    nbs <- 1:DBI::dbGetQuery(conn, tmp.query)[1, 1]
    if (isTRUE(bands)) {
        bands <- nbs
    }
    ...
    tmp.query.sr <- paste0("SELECT r_proj4 AS p4s FROM ", nameque, 
        ";")
    try(db.proj4 <- DBI::dbGetQuery(conn, tmp.query.sr)$p4s, silent = TRUE)
    if (!exists("db.proj4")) {
        tmp.query.sr <- paste0("SELECT proj4text AS p4s FROM spatial_ref_sys WHERE srid = ", 
            srid$st_srid, ";")
        db.proj4 <- DBI::dbGetQuery(conn, tmp.query.sr)$p4s
    }
    if (!is.null(db.proj4)) {
    ...
    if ("band_names" %in% rpostgis:::dbTableInfo(conn, name)$column_name) {
        try({
            ct <- 1
            for (b in bands) {
                lnm <- DBI::dbGetQuery(conn, paste0("SELECT DISTINCT band_names[", 
