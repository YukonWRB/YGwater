...
#'
#' Generate plots of water level/flow, snow pillows, or other continuous data present in the local hydrometric database generated by package HydroMetDB. For any given start/end day of year. Integrates return periods if requested. Intended to supersede [WRBfloods::levelPlot()] and [WRBfloods::flowPlot()]: see details.
#'
#' Notice: in many cases, you're better off using the Shiny app at [hydroApp()] to generate and export your plot. Read on if you need additional control over the final product. (Note on 2023-10-18: hydroApp is not yet set up to pull from the new DB, it still uses WRBtools and WRBfloods functions.)
#'
                               ...
                               con = hydrometConnect())
{
  ...
  if (!is.infinite(minHist)){
    line1 <- paste0("\n         \n         \n        Historical range based\n        on years ", lubridate::year(min(daily$datetime)), " to ", lubridate::year(max(realtime$datetime))-1, "." )
  } else {
    line1 <- "\n         \n         \n        Not enough data for\n        historical ranges"
