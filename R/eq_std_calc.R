...
eq_std_calc <- function(sampledata, calcs) {
  ...
  if (!all(is.na(sampledata$`Hard-D (mg/L)`))) {
    ...
    !is.na(
      mean(stats::na.omit(sampledata$`Ca-D (mg/L)`)) *
        mean(stats::na.omit(sampledata$`Mg-D (mg/L)`))
    )
  ) {
    Ca <- sampledata$`Ca-D (mg/L)`
    Mg <- sampledata$`Mg-D (mg/L)`
    hard <- 2.497 *
      mean(Ca[Ca != 0 & !is.na(Ca)]) +
      4.118 * mean(Mg[Mg != 0 & !is.na(Mg)])
  } else if (!all(is.na(sampledata$`Hard-T (mg/L)`))) {
    vec <- sampledata$`Hard-T (mg/L)`
    hard <- mean(vec[vec != 0 & !is.na(vec)])
  } else if (
    !is.na(
      mean(stats::na.omit(sampledata$`Ca-T (mg/L)`)) *
        mean(stats::na.omit(sampledata$`Mg-T (mg/L)`))
    )
  ) {
  ...
  if (!all(is.na(sampledata$`Temp-F (C)`))) {
    temp <- round_any(
      mean(stats::na.omit(sampledata$`Temp-F (C)`)),
      5,
      f = floor
    )
  } else {
  ...
  if (any(is.na(c(DOC, pH, hard)))) {
    ...
    pH < 6.5 | pH > 8.13 | hard < 23.4 | hard > 399 | DOC < 0.3 | DOC > 22.9
  ) {
    CCME_Zn_lt <- NA
  } else {
    CCME_Zn_lt <- exp(
      (0.947 * log(hard)) - (0.815 * pH) + (0.398 * log(DOC)) + 4.625
    ) /
      1000
  }
  ...
  if (any(is.na(c(DOC, pH, hard)))) {
    ...
    pH < 6.5 | pH > 8.13 | hard < 13.8 | hard > 250.5 | DOC < 0.3 | DOC > 17.3
  ) {
    CCME_Zn_st <- NA
  } else {
