    ...
    words_capitalized <- mapply(
      function(word, index) {
        ...
        if (word.sub %in% all_uppercase_words) {
          ...
          if (apostrophe) {
            #The first letter and the letter after the apostrophe should be capitalized
          ...
          #The rest of the words should be capitalized if in English. In French, only if they are specified as uppercase word
          if (language == "en") {
            return(stringr::str_to_title(word))
          } else if (language == "fr") {
      ...
      if (length(words_capitalized) > 1) {
        for (i in 1:(length(words_capitalized) - 1)) {
          ...
          if (stripped %in% french_keywords) {
            if (
              !(words_capitalized[[i + 1]] %in%
                c("de", "du", "des", "la", "le", "les"))
            ) {
              words_capitalized[[
                i + 1
              ]] <- stringr::str_to_title(words_capitalized[[i + 1]])
            } else {
              if (i < (length(words_capitalized) - 1)) {
                if (grepl("'", words_capitalized[[i + 2]])) {
                  before_apostrophe <- substr(words_capitalized[[i + 2]], 1, 2)
                  char_after_apostrophe <- toupper(substr(
                    words_capitalized[[i + 2]],
                    3,
                    3
                  ))
                  after_apostrophe <- substr(
                    words_capitalized[[i + 2]],
                    4,
                    nchar(words_capitalized[[i + 2]])
                  )
                  words_capitalized[[i + 2]] <- paste0(
                    before_apostrophe,
                    char_after_apostrophe,
                    after_apostrophe
                  )
                  ...
                  words_capitalized[[
                    i + 2
                  ]] <- stringr::str_to_title(words_capitalized[[i + 2]])
                }
              }
            }
          }
