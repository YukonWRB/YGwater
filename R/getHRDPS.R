...
getHRDPS <- function(clip = c("YT"), save_path = "choose", param = "APCP_Sfc") {
  # Save path
  if (save_path == "choose") {
    if (!interactive()) {
  ...
  latest_run <- as.character(lubridate::floor_date(
    current_utc - 4 * 60 * 60,
    "6 hours"
  ))
  ...
  tryCatch(
    {
      for (i in 1:48) {
        if (nchar(i) == 1) {
          # format i so that it works in creating the url
          i <- paste0("0", i)
        }
  ...
  if (failed) {
    ...
    previous_run <- as.character(lubridate::floor_date(
      current_utc - 8 * 60 * 60,
      "6 hours"
    ))
