...
createSnowTemplate <- function(
  ...
  for (i in 1:nrow(circuits)) {
    ...
    template <- openxlsx::loadWorkbook(system.file(
      "snow_survey/SnowSurveyTemplate.xlsx",
      package = "YGwater"
    )) # reloaded each time to start from original
    ...
    for (c in 1:length(sheet_names)) {
      ...
      openxlsx::cloneWorksheet(
        template,
        sheetName = sheet_names[c],
        clonedSheet = sheet_name
      )
      # Fill in worksheet
      openxlsx::writeData(
        template,
      ...
      for (m in maint$maintenance) {
        ...
        if (m == "Brush helipad/access trail") {
          openxlsx::writeData(
            template,
            sheet = sheet_names[c],
            x = "x",
        ...
        if (m == "Replace marker plate/plates") {
          openxlsx::writeData(
            template,
            sheet = sheet_names[c],
            x = "x",
    ...
    summary <- SWE_station(
      year = as.numeric(substr(target_date, start = 1, stop = 4)),
    ...
    summary <- summary[
      summary$location_name %in% courses,
      c("location_name", "location_id", "swe_prevyear", "swe_med")
    ]
    ...
    summary <- rbind(
      ...
      data.frame(
        "location_name" = locs,
        "location_id" = ids,
        "swe_prevyear" = rep(NA, times = length(locs)),
        "swe_med" = rep(NA, times = length(locs))
      )
    )
    ...
    for (s in 1:length(courses)) {
      openxlsx::writeFormula(
        template,
        sheet = "Summary",
        x = paste0("=IFERROR('", sheet_names[s], "'!G25*10, \"\")"),
        startCol = 6,
        startRow = 2 + s
      )
    }
    ...
    for (s in 1:length(courses)) {
      openxlsx::writeFormula(
        template,
        sheet = "Summary",
        x = paste0("=IFERROR(F", 2 + s, "/H", 2 + s, "*100, ", '"")'),
        startCol = 9,
        startRow = 2 + s
      )
    }
    ...
    for (s in 1:length(courses)) {
      openxlsx::writeFormula(
        template,
        sheet = "Summary",
        x = paste0("=IF(ISBLANK('", sheet_names[s], "'!L25), ", '"no", "yes")'),
        startCol = 11,
        startRow = 2 + s
      )
    }
