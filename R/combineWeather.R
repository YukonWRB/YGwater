...
combineWeather <- function(
  ...
  for (s in 1:length(stations)) {
    ...
    if (is.data.frame(stations[[s]])) {
      ...
      if (is.null(datetime_col)) {
        stop(
          "If the list elements in the 'stations' parameter are data.frames, the 'datetime_col' parameter must be specified."
        )
      }
    } else if (is.character(stations[[s]])) {
      if (is.null(interval)) {
        stop(
          "If the list elements in the 'stations' parameter are character strings, the 'interval' parameter must be specified."
        )
      }
      if (interval == "hour") {
        datetime_col <- "time"
      ...
      warning(
        "The list elements in the 'stations' parameter must be either station ids (character string) or data.frames"
      )
    }
    ...
    if (nrow(station) == 0) {
      warning(paste0(
        "Station ",
        station,
        " does not have data for this time frame"
      ))
    } else {
      # Subset to variables of interest and months of interest
      station <- station[,
        c(datetime_col, intersect(variables, names(station))),
        drop = FALSE
      ]
      station <- station %>%
  ...
  for (s in 2:length(stations)) {
    ...
    combined_variables <- stats::setNames(
      data.frame(matrix(ncol = 4, nrow = 0)),
      c(datetime_col, "value", "variable", "station")
    ...
    for (v in variables) {
      ...
      bias <- mean(overlap$value.x, na.rm = TRUE) -
        mean(overlap$value.y, na.rm = TRUE)
      message(paste0("Bias: ", bias))
      message(paste0(
        "RMSE: ",
        sqrt(mean((overlap$value.x - overlap$value.y)^2, na.rm = TRUE))
      ))
      ...
      if (inherits(stn2[[datetime_col]], "Date")) {
        all_dates <- seq.Date(
          as.Date(min(stn1[[datetime_col]], stn2[[datetime_col]])),
          as.Date(max(stn1[[datetime_col]], stn2[[datetime_col]])),
          by = "days"
        )
        ...
        all_dates <- seq.POSIXt(
          from = as.POSIXct(min(stn1[[datetime_col]], stn2[[datetime_col]])),
          to = as.POSIXct(max(stn1[[datetime_col]], stn2[[datetime_col]])),
          by = interval
        )
      ...
      if (length(missing_dates) >= 1) {
        ...
        stn1 <- dplyr::bind_rows(
          stn1[, c(datetime_col, "value", "variable", "station")],
          missing_data
        )
      }
      ...
      combined_data <- stn1 %>%
        dplyr::left_join(
          stn2,
          by = datetime_col,
          suffix = c("_stn1", "_stn2")
        ) %>%
        dplyr::mutate(
          value = dplyr::coalesce(.data$value_stn1, .data$value_stn2),
          variable = .data$variable_stn1,
          station = ifelse(
            !is.na(.data$value_stn1),
            .data$station_stn1,
            .data$station_stn2
          )
        ) %>%
  ...
  combined_stations <- combined_stations[
    order(combined_stations[[datetime_col]]),
  ] # Order by datetime
