...
waterInfo <- function(
  ...
  if (level_flow == "one") {
    for (i in unique(locs$location)) {
      ...
      if (nrow(sub) > 1) {
        if (
          params[params$param_name == "water flow", "parameter_id"] %in%
            sub$parameter_id
        ) {
          level_end <- sub[
            sub$parameter_id ==
              params[params$param_name == "water level", "parameter_id"],
            "end_datetime"
          ]
          ...
          flow_end <- sub[
            sub$parameter_id ==
              params[params$param_name == "water flow", "parameter_id"],
            "end_datetime"
          ]
          if (flow_end > end_date) {
            flow_end <- end_date
          }
          if (flow_end > level_end - 6 * 30 * 24 * 60 * 60) {
            # check if last flow is recent enough
            locs <- locs[
              !(locs$location == i &
                locs$parameter_id ==
                  params[params$param_name == "water level", "parameter_id"]),
  ...
  for (i in names(extremes)) {
    ...
    tryCatch(
      {
        gaps_min <- seq(min(yrs_min), max(yrs_min))[
          !(seq(min(yrs_min), max(yrs_min)) %in% yrs_min)
        ]
      },
      error = function(e) {
        gaps_min <<- NA
    ...
    tryCatch(
      {
        gaps_max <- seq(min(yrs_max), max(yrs_max))[
          !(seq(min(yrs_max), max(yrs_max)) %in% yrs_max)
        ]
      },
      error = function(e) {
        gaps_max <<- NA
    ...
    trends <- rbind(
      ...
      data.frame(
        ...
        "prct_annual_chg_min" = round(
          unname(minimum$estimates) / unname(min_intercept),
          3
        ),
        "n_max" = unname(maximum$parameter),
        "p.value_max" = round(unname(maximum$p.value), 3),
        "sens.slope_max" = round(unname(maximum$estimates), 3),
        "prct_annual_chg_max" = round(
          unname(maximum$estimates) / unname(max_intercept),
          3
        ),
        "note" = paste0(
          "Percent annual change based on intercepts of ",
          round(min_intercept, 1),
          " at the start year for miniumum values and ",
          round(max_intercept, 1),
          " at the start year for maximum values"
        )
      )
    )
    ...
    if (plots) {
      ...
      x_lim <- c(
        min(min(yrs_min), min(yrs_max)),
        max(max(yrs_min), max(yrs_max))
      )
      plot <- ggplot2::ggplot(
        data = tbl,
        ggplot2::aes(x = .data$Year, y = .data$Min_1_Day)
      ) +
      ...
      if (plot_type == "combined") {
        ...
        if (
          params[params$parameter_id == sub(".*_", "", i), "param_name"] ==
            "water flow"
        ) {
          plot <- plot +
            ggplot2::labs(
              y = "Annual extreme value (log scale)",
              title = paste0(sub("_.*", "", i), ": ", name),
              subtitle = paste0(
                "Parameter: ",
                params[params$parameter_id == sub(".*_", "", i), "param_name"],
                ", m3/s"
          ...
          plot <- plot +
            ggplot2::labs(
              y = "Annual extreme value",
              title = paste0(sub("_.*", "", i), ": ", name),
              subtitle = paste0(
                "Parameter: ",
                params[params$parameter_id == sub(".*_", "", i), "param_name"],
                ", m relative to benchmark"
              )
            )
        }
        ...
        plot2 <- ggplot2::ggplot(
          data = tbl,
          ggplot2::aes(x = .data$Year, y = .data$Max_1_Day)
        ) +
          ggplot2::labs(
            y = "Annual Maximum",
            title = paste0(sub("_.*", "", i), ": ", name),
        ...
        plots_separate <- cowplot::plot_grid(
          plot2,
          plot,
          ncol = 1,
          align = "v",
  ...
  if (plot_type == "combined" & plots & !quiet) {
    message(
      "Minimum and maximum flow plots were combined and returned in a list element. You can view each combined plot by calling grid::grid.draw on the desired object, or dig a bit deeper and find each individual ggplot object."
    )
  }
  ...
  if (!is.null(save_path)) {
    openxlsx::write.xlsx(
      result,
      paste0(save_path, "/WaterInfo_", Sys.Date(), "/measurements+stats.xlsx")
    )
  }
