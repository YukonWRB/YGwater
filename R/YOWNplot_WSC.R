...
YOWNplot_WSC <- function(
  ...
  plotdf_WSC <- plotdf_WSC %>%
    dplyr::filter(
      .data$timestamp_MST >= min(stats::na.omit(plotdf_YOWN$timestamp_MST))
    )

  # Create one massive data frame for chart parameter calculation
  fulldf <- dplyr::full_join(
    plotdf_YOWN,
    plotdf_WSC,
    by = c("ID", "timestamp_MST", "value")
  )

  diff <- as.numeric(difftime(
    max(fulldf$timestamp),
    min(fulldf$timestamp),
    units = "days"
  ))
  ...
  caption <- ggplot2::ggplot() +
    ...
    ggplot2::labs(
      title = paste0(
        "Period of Record: ",
        strftime(
          as.POSIXct(min(stats::na.omit(plotdf_YOWN$timestamp_MST))),
          format = "%Y-%m-%d"
        ),
        " to ",
        strftime(
          as.POSIXct(max(stats::na.omit(plotdf_YOWN$timestamp_MST))),
          format = "%Y-%m-%d"
        ),
        "\nPlot generated: ",
        Sys.Date(),
  ...
  subtitle <- ggplot2::ggplot() +
    ...
    ggplot2::labs(
      title = paste0(
        "YOWN Sites: ",
        paste(YOWNindex, collapse = ", "),
        "\n",
        "WSC Sites: ",
        paste(WSCindex, collapse = ", ")
      )
    ) +
    ggplot2::theme(
      plot.title = ggplot2::element_text(
