...
YOWNplot_SiteCompare <- function(
  ...
  if (tsunit == "asl") {
    ...
    print("Please double check the type of plot specified")
  }
  ...
  for (i in YOWNindex) {
    ...
    timeseries$lag_val <- difftime(
      timeseries$timestamp_MST,
      timeseries$ts_lag,
      units = "hours"
    )
    gapdf <- timeseries %>%
      ...
      for (j in 1:nrow(gapdf)) {
        df <- data.frame(
          seq.POSIXt(
            from = gapdf[j, 1],
            by = "-1 hour",
            length.out = gapdf[j, 8]
          ),
          NA,
          as.character(-5),
          "MISSING DATA",
          gapdf$approval_level[j],
          gapdf$approval_description[j],
          NA,
          NA
        )
  ...
  plot <- ggplot2::ggplot() +
    ...
    ggplot2::guides(
      color = ggplot2::guide_legend(override.aes = list(size = 1.5), nrow = 1)
    ) +
  ...
  if (tsunit == "asl") {
    limits <- c(
      round_any(min(stats::na.omit(plotdf$value)), 0.5, f = floor),
      round_any(max(stats::na.omit(plotdf$value)), 0.5, f = ceiling)
    )
    breaks <- seq(
      floor(min(stats::na.omit(plotdf$value))),
      ceiling(max(stats::na.omit(plotdf$value))),
      by = 0.25
    )
    ...
    plot <- plot +
      ggplot2::scale_y_reverse(
        name = paste0("Groundwater level (m ", tsunit, " )"),
        limits = limits,
        breaks = breaks,
        expand = c(0, 0)
      )
  ...
  title <- ggplot2::ggplot() +
    ...
    ggplot2::labs(
      title = "Groundwater Level Record: Site Comparison - Cowley Creek YOWN-1930"
    ) +
    ggplot2::theme(
  ...
  caption <- ggplot2::ggplot() +
    ...
    ggplot2::labs(
      title = paste0(
        "Period of Record: ",
        strftime(
          as.POSIXct(min(stats::na.omit(timeseries$datetime))),
          format = "%Y-%m-%d"
        ),
        " to ",
        strftime(
  ...
  subtitle <- ggplot2::ggplot() +
    ...
    ggplot2::theme_minimal() +
    ggplot2::labs(
      title = paste0("Source Data: Aquarius Time Series, ", tsunit)
    ) +
    ggplot2::theme(
      plot.title = ggplot2::element_text(
        hjust = 0,
  ...
  final_plot <- cowplot::ggdraw() +
    cowplot::draw_image(
      "G:\\water\\Groundwater\\2_YUKON_OBSERVATION_WELL_NETWORK\\4_YOWN_DATA_ANALYSIS\\1_WATER_LEVEL\\00_AUTOMATED_REPORTING\\01_MARKUP_IMAGES\\Template_nogrades.jpg"
    ) +
    cowplot::draw_plot(final)

  ggplot2::ggsave(
    plot = final_plot,
    filename = paste0(saveTo, "SiteCompare.pdf"),
    height = 8.5,
    width = 11,
    units = "in"
  )
