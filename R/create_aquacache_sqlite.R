...
#'
#' This helper connects to a running AquaCache PostgreSQL database and writes a
#' small subset of records to a SQLite file.  It is intended for package tests
#' or examples where a local database is required but only a few locations are
#' needed.
#'
#' Only the specified locations are kept from the source database.  Continuous
#' data is copied from `continuous.timeseries` and
#' `continuous.measurements_continuous` while discrete data is copied from
#' `discrete.samples` and `discrete.results`.  Several lookup tables used by
#' package plotting functions (e.g. `grade_types`, `approval_types`,
#' `qualifier_types`) are also copied in full so the resulting database is
#' largely self contained.
#'
#' @param con An open connection to the AquaCache PostgreSQL database.  Use
#'   [AquaConnect()] to create this connection.
#' @param file Path to the SQLite file to create.
#' @param continuous_locations Character vector of location codes to retain in
#'   continuous tables.  Defaults to the first location found in the database
#'   when `NULL`.
#' @param discrete_locations Character vector of location codes to retain in
#'   discrete tables.  Defaults to `continuous_locations` when `NULL`.
#' @param overwrite Logical. Overwrite `file` if it already exists.
#'
#' @return The path to the created SQLite file, invisibly.
#' @examples
#' \dontrun{
#' pg <- AquaConnect()
...
create_aquacache_sqlite <- function(con, file,
  ...
  # add additional lookup tables used by plotting helpers
  ref_tables <- c(
    "public.parameters",
    "public.media",
    "public.media_types",
  ...
  # Continuous data --------------------------------------------------------
  if (length(continuous_locations)) {
    ...
    if (nrow(ts)) {
      ...
      corrections <- DBI::dbGetQuery(con,
        sprintf("SELECT * FROM continuous.corrections WHERE timeseries_id IN (%s)",
                ts_ids))
      DBI::dbWriteTable(sqlite, "corrections", corrections)
    }
  }

  # Discrete data ---------------------------------------------------------
  if (length(discrete_locations)) {
    ...
    series <- DBI::dbGetQuery(con,
      sprintf("SELECT * FROM discrete.sample_series WHERE location_id IN (%s)",
              loc_ids))
    if (nrow(series)) {
      DBI::dbWriteTable(sqlite, "sample_series", series)
    }
  }
