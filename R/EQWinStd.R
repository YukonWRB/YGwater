...
EQWinStd <- function(CalcIds, SampleIds, con = NULL) {
  ...
  calc_query <- paste0(
    "SELECT CalcId, CalcScript FROM eqcalcs WHERE CalcId IN (",
    paste(CalcIds, collapse = ","),
    ");"
  )
  ...
  sample_query <- paste0(
    "SELECT SampleId, CollectDateTime FROM eqsampls WHERE SampleId IN (",
    paste(SampleIds, collapse = ","),
    ");"
  )
  ...
  replace_params <- function(expression, params, sample_date) {
    ...
    expression <- gsub(
      "#(y{4})",
      paste0("#", format(sample_date, "%Y")),
      expression
    )
    expression <- gsub(
      "-m{2}-",
      paste0("-", format(sample_date, "%m"), "-"),
      expression
    )
    expression <- gsub(
      "-d{2}#",
      paste0("-", format(sample_date, "%d"), "#"),
      expression
    )
  ...
  process_script <- function(script_lines, params, sample_date) {
    ...
    for (line in script_lines) {
      ...
      if (grepl("\\?", expr)) {
        ...
        false_value <- ifelse(
          length(strsplit(strsplit(expr, "\\?")[[1]][2], ":")[[1]]) > 1,
        ...
        if (condition_result) {
          ...
          if (!is.na(false_value)) {
            false_value_quoted <- regmatches(
              false_value,
              gregexpr('"[^"]*"', false_value)
            )[[1]]
  ...
  for (calc_id in CalcIds) {
    ...
    for (j in seq_along(SampleIds)) {
      ...
      # Get sample date
      sample_date <- samples_df$CollectDateTime[
        samples_df$SampleId == sample_id
      ]
