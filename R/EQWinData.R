...
EQWinData <- function(
  ...
  if (!is.null(DL1)) {
    if (!DL1 %in% c("negative", "half", "NA")) {
      stop("DL1 must be NULL, 'negative', 'half', or 'NA'")
    }
  }
  if (!is.null(DL2)) {
    if (!DL2 %in% c("convert", "NA")) {
      stop("DL2 must be NULL, 'convert', or 'NA'")
    }
  }
  ...
  if (!is.null(parameters)) {
    ParamIds <- DBI::dbGetQuery(
      EQWin,
      paste0(
        "SELECT ParamId, ParamCode, Units FROM eqparams WHERE ParamCode IN ('",
  ...
  sampleIds <- DBI::dbGetQuery(
    EQWin,
    paste0(
      "SELECT eqsampls.StnId, eqsampls.SampleId, eqsampls.CollectDateTime, eqsampls.SampleClass, eqcodes.CodeDesc FROM eqsampls INNER JOIN eqcodes ON eqsampls.SampleClass = eqcodes.CodeValue WHERE eqcodes.CodeField = 'eqsampls.SampleClass' AND eqsampls.StnId IN (",
  ...
  results <- DBI::dbGetQuery(
    EQWin,
    paste0(
      "SELECT eqdetail.SampleId, eqdetail.ParamId, eqdetail.Result, eqparams.ParamCode, eqparams.ParamName, eqparams.Units FROM eqdetail INNER JOIN eqparams ON eqdetail.ParamId = eqparams.ParamId WHERE eqdetail.SampleId IN (",
  ...
  if (format == 'wide') {
    ...
    for (i in unique(samps_locs$StnId)) {
      # Making one tab per station
      location <- samps_locs[samps_locs$StnId == i, ]
      ids <- sampleIds[sampleIds$StnId == i, ]
      datastn <- data.frame()
      for (j in 1:nrow(ids)) {
        # This will add one column per sample ID
        id <- ids$SampleId[j]
        rawdata <- results[
          results$SampleId == id,
          c("Result", "ParamName", "SampleId")
        ]
        if (nrow(rawdata) == 0) {
          next
        }
        rawdata <- merge(
          rawdata,
          ids[, c("CollectDateTime", "CodeDesc", "SampleId")]
        )
        data <- data.frame(Parameter = unique(rawdata$ParamName), tmp = NA)
        names(data) <- c(
          "Parameter",
          paste0(
            as.character(rawdata$CollectDateTime[1]),
            " ",
            rawdata$CodeDesc[1]
          )
        )
    ...
    openxlsx::write.xlsx(
      datalist,
      paste0(save_path, "/EQWinData_wide ", write_time, ".xlsx")
    )
    message(
      "Data workbook saved to ",
      paste0(save_path, "/EQWinData_wide ", write_time, ".xlsx")
    )
    ...
    datalist[["data"]] <- merge(
      ...
      samps_locs[, c("SampleId", "StnId", "CollectDateTime", "SampleClass")]
    ) # added SampleClass
    if (!is.null(DL1)) {
      if (DL1 == "negative") {
        datalist[["data"]]$Result <- sapply(
    ...
    datalist[["locations"]] <- locations[
      locations$StnId %in% datalist[["data"]]$StnId,
      c("StnId", "StnName", "StnDesc")
    ]
    datalist[["parameters"]] <- unique(results[, c(
      "ParamId",
      "ParamName",
      "Units"
    )])
    openxlsx::write.xlsx(
      datalist,
      paste0(save_path, "/EQWinData_long", write_time, ".xlsx"),
      overwrite = TRUE
    )
    message(
      "Data workbook saved to ",
  ...
  if (!is.null(stds)) {
    ...
    for (i in 1:nrow(standards)) {
      standards_list[[standards$StdCode[i]]] <- stdVals[
        stdVals$StdId == standards$StdId[i],
        c("ParamName", "ParamId", "MinVal", "MaxVal")
      ]
    }
    openxlsx::write.xlsx(
      standards_list,
      paste0(save_path, "/EQWinStandards ", write_time, ".xlsx")
    )
    message(
      "Standards workbook saved to ",
      paste0(save_path, "/EQWinStandards ", write_time, ".xlsx")
    )
  ...
  if (stnStds) {
    tmp <- DBI::dbGetQuery(
      ...
      paste0(
        ...
        paste0(unique(samps_locs$StnId), collapse = ", "),
    ...
    if (nrow(tmp) == 0) {
      ...
      tmp <- merge(
        ...
        DBI::dbGetQuery(
          EQWin,
          paste0(
            "SELECT StdCode, StdDesc, StdId FROM eqstds WHERE StdCode IN ('",
      ...
      for (i in unique(tmp$StdCode)) {
        ...
        standard <- DBI::dbGetQuery(
          EQWin,
          paste0(
            "SELECT ParamId, MaxVal, MinVal, StdValNote FROM eqstdval WHERE StdId = ",
            unique(tmp[tmp$StdCode == i, "StdId"]),
            ";"
          )
        )
      ...
      openxlsx::write.xlsx(
        stn_standards_list,
        paste0(save_path, "/EQWinStnStandards ", write_time, ".xlsx")
      )
      message(
        "Stations standards workbook saved to ",
        paste0(save_path, "/EQWinStnStandards ", write_time, ".xlsx")
      )
