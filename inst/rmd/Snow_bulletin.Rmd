...
calcCDDF <- function(temps) {
  cddf <- 0
  temps$cddf <- NA
  for (d in 1:length(temps$mean_temp)) {
    t <- temps$mean_temp[d]
...
temps <- temps[format(temps$date, "%m") %in% c("09", "10", "11", "12", "01", "02", "03", "04", "05", "06"),]

# Find first and last year 
first_yr <- format(min(temps$date), "%Y")
last_yr <- format(max(temps$date), "%Y")

cddf <- data.frame()
# Run over every year
for (y in first_yr:last_yr) {
  # Subset data
    tab <- temps[temps$date >= paste0(y, '-09-01') 
                 & temps$date < paste0(y+1, '-06-14'),]
  # Only calculate if missing less than 10 days, but only for years that are not in the 'years' list
    if (sum(!is.na(tab$mean_temp)) >= 276 | format(min(tab$date), "%Y") %in% year) {
      cddf_y <- calcCDDF(tab)
...
cddf <- cddf[,c("date", "cddf")]
colnames(cddf) <- c("datetime", "value")

return(cddf)
}

# Function for creating hydrometContinuous input file, specific to CDDF
makeContinuousData <- function(table, year) {
...
cddf$day <- format(cddf$date, "%m-%d")
summary_cddf <- cddf %>%
  dplyr::group_by(day) %>%
  dplyr::summarise(min = round(min(cddf), 0),
            max = round(max(cddf), 0),
            md = round(median(cddf), 0),
            q75 = round(quantile(cddf, 0.75), 0),
            q25 = round(quantile(cddf, 0.25), 0))

# Add date
summary_cddf$date <- NA
# For days between 09-01 and 12-31, date is year. For days between 01-01 and 08-31, date is year + 1
summary_cddf[summary_cddf$day >= "09-01" & summary_cddf$day <= "12-31",]$date <- paste0(year, "-", summary_cddf[summary_cddf$day >= "09-01" & summary_cddf$day <= "12-31",]$day )  
summary_cddf[summary_cddf$day >= "01-01" & summary_cddf$day <= "08-31",]$date <- paste0(year+1, "-", summary_cddf[summary_cddf$day >= "01-01" & summary_cddf$day <= "08-31",]$day )
summary_cddf$date <- as.POSIXct(summary_cddf$date, format="%Y-%m-%d")

# Add columns for year and value. If want to add additional years, then would only need to add value, not stats
tab <- merge(summary_cddf, cddf[cddf$date >= paste0(year, '-09-01')
                 & cddf$date < paste0(year+1, '-06-14'),], by=c("date", "day"), all.x = TRUE)


# Add columns to imitate hydrometContinuous realtime table
tab$year <- format(tab$date, "%Y")
tab$month <- format(tab$date, "%m")
tab$day <- format(tab$date, "%d")
tab$plot_year <- paste0(year, "-", year+1)
tab$date <- as.POSIXct(tab$date, format="%Y-%m-%d")
tab$fake_datetime <- tab$date

# rename columns
colnames(tab) <- c("datetime", "day", "min", "max", "md", "q75", "q25", "mean_temp", "value", "year", "month", "plot_year", "fake_datetime")
...
# D
chooseWeather(coords= c(60.7197, -135.0523), dist=7, interval='day', start=1950, end=2023, variable='mean_temp', months=c('01', '02', '03', '04', '05', '06', '07', '08', '09', 10, 11, 12), return_data = TRUE)
#Get climate data for Whitehorse
combined_data <- combineWeather(stations=list("2101310", "2101300"), start='1978-01-01', end='2023-11-19', variable="mean_temp", months=NULL)
combined_data <- combineWeather(stations=list(combined_data, "2101400"), start='1978-01-01', end='2023-11-19', variable="mean_temp", months=NULL)
combined_data <- combineWeather(stations=list(combined_data, "2101415"), start='1978-01-01', end='2023-11-19', variable="mean_temp", months=NULL)
combined_data <- combineWeather(stations=list(combined_data, "2101303"), start='1978-01-01', end='2023-11-19', variable="mean_temp", months=NULL)

Whitehorse <- combined_data

test <- getCDDF(Whitehorse, 2023)

# Plot
hydrometContinuous(parameter= "CDDF", years = c(2022), title = TRUE, custom_title = "Whitehorse CDDF", returns = "none", allowed_missing = 10, plot_scale = 1, save_path = NULL, con = hydrometConnect(), cddf_data = test)
...
# # D
# # Get climate data for Dawson
# combined_data <- combineWeather(stations=list("2100LRP", "2100402"), start='1978-01-01', end='2023-11-19', variable="mean_temp", months=NULL)
# combined_data <- combineWeather(stations=list(combined_data, "2100407"), start='1978-01-01', end='2023-11-19', variable="mean_temp", months=NULL)
# combined_data <- combineWeather(stations=list(combined_data, "2100400"), start='1978-01-01', end='2023-11-19', variable="mean_temp", months=NULL)
# combined_data <- combineWeather(stations=list(combined_data, "2101062"), start='1978-01-01', end='2023-11-19', variable="mean_temp", months=NULL)
# 
# Dawson <- combined_data
# 
 test <- makeContinuousData(Dawson, 2022)

hydrometContinuous(location="Dawson", parameter = "CDDF", startDay = NULL, endDay = NULL, tzone = "MST", years = c(2022), datum = TRUE, title = TRUE, custom_title = "Dawson CDDF", returns = "none", allowed_missing = 10, plot_scale = 1, save_path = NULL, con = hydrometConnect(), continuous_data = test)
