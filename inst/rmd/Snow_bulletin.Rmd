...
!!!Important note for Emilie: you'll need to include the following code in whatever R function you use to call this markdown report.
...
The reason for this is that knitr is called for within the .rmd and not the R script; since check() doesn't look at the .rmd, having knitr as a a "depends" in the DESCRIPTION file results in an error, but it's still needed for the function. Instead it's in the "suggests" section.
...
swe_basins <- SWE_basin(year <- 2023,
                                   month <- c(3, 4, 5), # c(3)
                                   threshold <- 6,
                                   csv = FALSE,
                                   summarise = FALSE)
