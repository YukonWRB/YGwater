...
# TODO: double assignment (<<-) is used in some places. Likely not necessary.
...
snowbullpillow2 <- function(location_pil, 
                            ...
                            scale = scale_param, 
                            month = month_param) {
  # Get this years snow pillow data
  ...
  snow_s <- DBI::dbGetQuery(con, paste0("SELECT s.target_datetime, r.result AS value FROM results r
                                      ...
                                      WHERE s.location_id = ", location_surv, " 
                                      AND r.parameter_id = 21"))
  
    # snow_s <- DBI::dbGetQuery(con, paste0("SELECT s.target_datetime, r.result AS value FROM results r
    #                                   JOIN samples s ON r.sample_id = s.sample_id
    #                                   WHERE s.location_id = ", location_surv, "
    #                                   AND r.parameter_id = 21 
    #                                   AND s.target_datetime > '", year - lookback, "-03-01';"))
    
  snow_s$fake_date <- as.Date(paste0(year, "-", format(snow_s$target_datetime, "%m-%d")))
  ...
  plot <- ggplot2::ggplot() +
    ...
    ggplot2::scale_x_datetime(date_breaks = "2 months", labels = custom_date_labels, expand = c(0, 0), 
                              limits = c(as.POSIXct(paste0(year - 1, "-09-01"), tz = "UTC"), as.POSIXct(paste0(year, "-06-30"), tz = "UTC"))) + 
    ggplot2::scale_y_continuous(limits = c(min(min(snow_p$value), min(snow_s$value)), max(max(snow_p$value), max(snow_s$value))), expand = c(0.01, 0.05)) +
...
snowBullPrecip <- function(tsid, year, scale = scale_param, custom_title, language, month = month_param) {
    tab <- DBI::dbGetQuery(con, paste0("SELECT timeseries_id, datetime, value_corrected AS value, period, imputed
                                       FROM measurements_continuous_corrected 
                                       WHERE timeseries_id = ", tsid,
    ...
    #                                    " AND datetime >= '", year - lookback, "-10-01'",
    #                                    " AND datetime <= '", year, "-0", month_param, "-02'"))
...
snowbullCDDF <- function(timeseries_id, 
  ...
  #                                       WHERE timeseries_id = ", timeseries_id, 
  #                                       " AND datetime <= '", year, "-0", month, "-01'
  #                                       AND datetime >= '", year - lookback, "-01-01';"))
...
-   **Figure A:** Daily Snow Water Equivalent (SWE) data starting in September at one specific location in the watershed, showing an overview of winter snowpack evolution compared with historical data for `r if (snow_period == "last 40 years") paste0("the ", snow_period) else if (snow_period == "all years") paste0(snow_period, " of record") else paste0("years ", snow_period)`.
-   **Figure B:** Current basin-averaged estimated Snow Water Equivalent (SWE) from snow survey data compared with historical data for `r if (snow_period == "last 40 years") paste0("the ", snow_period) else if (snow_period == "all years") paste0(snow_period, " of record") else paste0("years ", snow_period)`, serving as an indicator of potential runoff volumes in the spring (acknowledging that snow sublimation, evapotranspiration, rain and glacier melt also significantly affect runoff).
-   **Figure C:** Monthly winter precipitation (rain and/or snow) compared with historical data for `r if (precip_period == "last 40 years") paste0("the ", precip_period) else if (precip_period == "all years") paste0(precip_period, " of record") else paste0("years ", precip_period)`, complementing the information presented in Figure B.
-   **Figure D:** Cumulative degree-days of freezing (CDDF, sum of negative daily temperatures) compared with historical data for `r if (cddf_period == "last 40 years") paste0("the ", cddf_period) else if (cddf_period == "all years") paste0(cddf_period, " of record") else paste0("years ", cddf_period)`, functioning as an indicator of winter coldness and overall river ice thickness; variables that influence river ice breakup scenarios in the spring.
-   **Figure E:** Current estimated daily discharge or measured water level, compared with historical data for `r if (water_period == "last 40 years") paste0("the ", water_period) else if (water_period == "all years") paste0(water_period, " of record") else paste0("years ", water_period)`, representing an overview of the watershed hydrological conditions. The flood level refers to the lowest elevation at which flood impacts are estimated to occur.
...
`r officer::ftext(paste0(if (nchar(stats$basin_stats[stats$basin_stats$basin=="Upper_Yukon",]$description) > 0) paste0(stats$basin_stats[stats$basin_stats$basin=="Upper_Yukon",]$description, " "),
...
# Plot SWE
if (language == "french") {
  snowbullpillow(location = station_id, custom_title = "A2 : Équivalent en eau de la neige - Wolf Creek", year = year_param, month = month_param, language = "fr", scale = scale_param)
} else {
  snowbullpillow(location = station_id, custom_title = "A2: Wolf Creek Subalpine Snow Water Equivalent", year = year_param, month = month_param, language = "en", scale = scale_param)
}
...
`r officer::ftext(paste0(if (nchar(stats$precip_stats[stats$precip_stats$location_id==48168 & stats$precip_stats$period == precip_period, "description"]) > 0) paste0(stats$precip_stats[stats$precip_stats$location_id==48168 & stats$precip_stats$period == precip_period, "description"], " "), "normal"), textColour(stats$precip_stats[stats$precip_stats$location_id==48168 & stats$precip_stats$period==precip_period, "description"]))` from October through `r month.name[params$month-1]`
(Figure`r knitr::asis_output("\U00A0")`C). Cumulative winter
precipitation was
**`r stats$precip_stats[stats$precip_stats$location_id==48168 & stats$precip_stats$period==precip_period,]$perc_hist_med`%**
of historical median on `r month.name[params$month]` 1. Cumulative
degree-days of freezing (CDDF) are
**`r stats$cddf_stats[stats$cddf_stats$location_id==48168 & stats$cddf_stats$period==cddf_period,]$perc_hist_med`%**
of historical median, with
**`r stats$cddf_stats[stats$cddf_stats$location_id==48168 & stats$cddf_stats$period==cddf_period,]$value`$^\circ$C-Days**
on `r month.name[params$month]` 1
(Figure`r knitr::asis_output("\U00A0")`D), which suggests 
`r iceThickness(stats$cddf_stats[stats$cddf_stats$location_id==48168 & stats$cddf_stats$period==cddf_period,]$perc_hist_med) ` on rivers and lakes of the region.
...
Lake is currently 
`r officer::ftext(paste0(if (nchar(stats$flow_stats[stats$flow_stats$location_id=="09AB004",]$description) > 0) paste0(stats$flow_stats[stats$flow_stats$location_id=="09AB004",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$flow_stats[stats$flow_stats$location_id=="09AB010",]$description) > 0) paste0(stats$flow_stats[stats$flow_stats$location_id=="09AB010",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$basin_stats[stats$basin_stats$basin=="Teslin_Big_Salmon",]$description) > 0) paste0(stats$basin_stats[stats$basin_stats$basin=="Teslin_Big_Salmon",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$precip_stats[stats$precip_stats$location_id==8985 & stats$precip_stats$period==precip_period,]$description) > 0) paste0(stats$precip_stats[stats$precip_stats$location_id==8985 & stats$precip_stats$period==precip_period,]$description, " "), "normal"), textColour(stats$precip_stats[stats$precip_stats$location_id==8985 & stats$precip_stats$period==precip_period,]$description))` from October through `r month.name[params$month-1]`
(Figure`r knitr::asis_output("\U00A0")`C). Cumulative winter
precipitation was
**`r stats$precip_stats[stats$precip_stats$location_id==8985 & stats$precip_stats$period==precip_period,]$perc_hist_med`%**
of historical median on `r month.name[params$month]` 1. Cumulative
degree-days of freezing (CDDF) are
**`r stats$cddf_stats[stats$cddf_stats$location_id==8985 & stats$cddf_stats$period==cddf_period,]$perc_hist_med`%**
of historical median, with
**`r stats$cddf_stats[stats$cddf_stats$location_id==8985 & stats$cddf_stats$period==cddf_period,]$value`$^\circ$C-Days**
on `r month.name[params$month]` 1
(Figure`r knitr::asis_output("\U00A0")`D), which suggests `r iceThickness(stats$cddf_stats[stats$cddf_stats$location_id==8985 & stats$cddf_stats$period==cddf_period,]$perc_hist_med) `
...
`r officer::ftext(paste0(if (nchar(stats$flow_stats[stats$flow_stats$location_id=="09AE002",]$description) > 0) paste0(stats$flow_stats[stats$flow_stats$location_id=="09AE002",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$basin_stats[stats$basin_stats$basin=="Central_Yukon",]$description) > 0) paste0(stats$basin_stats[stats$basin_stats$basin=="Central_Yukon",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$precip_stats[stats$precip_stats$location_id==27950 & stats$precip_stats$period==precip_period,]$description) > 0) paste0(stats$precip_stats[stats$precip_stats$location_id==27950 & stats$precip_stats$period==precip_period,]$description, " "), "normal"), textColour(stats$precip_stats[stats$precip_stats$location_id==27950 & stats$precip_stats$period==precip_period,]$description))` from October through `r month.name[params$month-1]`
(Figure`r knitr::asis_output("\U00A0")`C). Cumulative winter
precipitation was
**`r stats$precip_stats[stats$precip_stats$location_id==27950 & stats$precip_stats$period==precip_period,]$perc_hist_med`%**
of historical median on `r month.name[params$month]` 1. Cumulative
degree-days of freezing (CDDF) are
**`r stats$cddf_stats[stats$cddf_stats$location_id==27950 & stats$cddf_stats$period==cddf_period,]$perc_hist_med`%**
of historical median, with
**`r stats$cddf_stats[stats$cddf_stats$location_id==27950 & stats$cddf_stats$period==cddf_period,]$value`$^\circ$C-Days**
on `r month.name[params$month]` 1
(Figure`r knitr::asis_output("\U00A0")`D), which suggests `r iceThickness(stats$cddf_stats[stats$cddf_stats$location_id==27950 & stats$cddf_stats$period==cddf_period,]$perc_hist_med) `
on rivers and lakes of the region.
...
`r officer::ftext(paste0(if (nchar(stats$flow_stats[stats$flow_stats$location_id=="09AH004",]$description) > 0) paste0(stats$flow_stats[stats$flow_stats$location_id=="09AH004",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$basin_stats[stats$basin_stats$basin=="Pelly",]$description) > 0) paste0(stats$basin_stats[stats$basin_stats$basin=="Pelly",]$description, " "),
...
# Plot SWE
if (language == "french") {
  snowbullpillow(location = station_id, custom_title = "A : Équivalent en eau de la neige à Twin Creeks", year = year_param, month = month_param, language = "fr")
} else {
...
**`r stats$cddf_stats[stats$cddf_stats$location_id==8964 & stats$cddf_stats$period==cddf_period,]$perc_hist_med`%**
of historical median, with
**`r stats$cddf_stats[stats$cddf_stats$location_id==8964 & stats$cddf_stats$period==cddf_period,]$value`$^\circ$C-Days**
on `r month.name[params$month]` 1
(Figure`r knitr::asis_output("\U00A0")`D), which suggests `r iceThickness(stats$cddf_stats[stats$cddf_stats$location_id==8964 & stats$cddf_stats$period==cddf_period,]$perc_hist_med) ` on rivers and lakes of the region.
...
`r officer::ftext(paste0(if (nchar(stats$flow_stats[stats$flow_stats$location_id=="09BC001",]$description) > 0) paste0(stats$flow_stats[stats$flow_stats$location_id=="09BC001",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$basin_stats[stats$basin_stats$basin=="Stewart",]$description) > 0) paste0(stats$basin_stats[stats$basin_stats$basin=="Stewart",]$description, " "),
...
# Plot SWE
if (language == "french") {
  snowbullpillow(location = station_id, custom_title = "A : Équivalent en eau de la neige au lac Withers", year = year_param, month = month_param, language = "fr")
} else {
...
`r officer::ftext(paste0(if (nchar(stats$precip_stats[stats$precip_stats$location_id==51426 & stats$precip_stats$period==precip_period,]$description) > 0) paste0(stats$precip_stats[stats$precip_stats$location_id==51426 & stats$precip_stats$period==precip_period,]$description, " "), "normal"), textColour(stats$precip_stats[stats$precip_stats$location_id==51426 & stats$precip_stats$period==precip_period,]$description))` from October through `r month.name[params$month-1]`
(Figure`r knitr::asis_output("\U00A0")`C). Cumulative winter
precipitation was
**`r stats$precip_stats[stats$precip_stats$location_id==51426 & stats$precip_stats$period==precip_period,]$perc_hist_med`%**
of historical median on `r month.name[params$month]` 1. Cumulative
degree-days of freezing (CDDF) are
**`r stats$cddf_stats[stats$cddf_stats$location_id==51426 & stats$cddf_stats$period==cddf_period,]$perc_hist_med`%**
of historical median, with
**`r stats$cddf_stats[stats$cddf_stats$location_id==51426 & stats$cddf_stats$period==cddf_period,]$value`$^\circ$C-Days**
on `r month.name[params$month]` 1
(Figure`r knitr::asis_output("\U00A0")`D), which suggests `r iceThickness(stats$cddf_stats[stats$cddf_stats$location_id==51426 & stats$cddf_stats$period==cddf_period,]$perc_hist_med) ` on rivers and lakes of the region.
...
`r officer::ftext(paste0(if (nchar(stats$flow_stats[stats$flow_stats$location_id=="09DD003",]$description) > 0) paste0(stats$flow_stats[stats$flow_stats$location_id=="09DD003",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$basin_stats[stats$basin_stats$basin=="White",]$description) > 0) paste0(stats$basin_stats[stats$basin_stats$basin=="White",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$flow_stats[stats$flow_stats$location_id=="09CB001",]$description) > 0) paste0(stats$flow_stats[stats$flow_stats$location_id=="09CB001",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$basin_stats[stats$basin_stats$basin=="Lower_Yukon",]$description) > 0) paste0(stats$basin_stats[stats$basin_stats$basin=="Lower_Yukon",]$description, " "),
...
## A King Solomon Dome
if (language_param == "french") {
  snowbullpillow2(location_pil = 85, location_surv = 136, custom_title="A : Équivalent en eau de la neige à King Solomon Dome" , year = year_param, scale = scale_param)
} else {
...
`r officer::ftext(paste0(if (nchar(stats$precip_stats[stats$precip_stats$location_id==10194 & stats$precip_stats$period==precip_period,]$description) > 0) paste0(stats$precip_stats[stats$precip_stats$location_id==10194 & stats$precip_stats$period==precip_period,]$description, " "), "normal"), textColour(stats$precip_stats[stats$precip_stats$location_id==10194 & stats$precip_stats$period==precip_period,]$description))` from October through `r month.name[params$month-1]`
(Figure`r knitr::asis_output("\U00A0")`C). Cumulative winter
precipitation was
**`r stats$precip_stats[stats$precip_stats$location_id==10194 & stats$precip_stats$period==precip_period,]$perc_hist_med`%**
of historical median on `r month.name[params$month]` 1. Cumulative
degree-days of freezing (CDDF) are
**`r stats$cddf_stats[stats$cddf_stats$location_id==10194 & stats$cddf_stats$period==cddf_period,]$perc_hist_med`%**
of historical median, with
**`r stats$cddf_stats[stats$cddf_stats$location_id==10194 & stats$cddf_stats$period==cddf_period,]$value`$^\circ$C-Days**
on `r month.name[params$month]` 1
(Figure`r knitr::asis_output("\U00A0")`D), which suggests `r iceThickness(stats$cddf_stats[stats$cddf_stats$location_id==10194 & stats$cddf_stats$period==cddf_period,]$perc_hist_med) ` on rivers and lakes of the region.
...
`r officer::ftext(paste0(if (nchar(stats$flow_stats[stats$flow_stats$location_id == "09CD001",]$description) > 0) paste0(stats$flow_stats[stats$flow_stats$location_id=="09CD001",]$description, " "), "average"), textColour(stats$flow_stats[stats$flow_stats$location_id=="09CD001",]$description))`
...
`r officer::ftext(paste0(if (nchar(stats$basin_stats[stats$basin_stats$basin=="Porcupine",]$description) > 0) paste0(stats$basin_stats[stats$basin_stats$basin=="Porcupine",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$precip_stats[stats$precip_stats$location_id==26894 & stats$precip_stats$period==precip_period,]$description) > 0) paste0(stats$precip_stats[stats$precip_stats$location_id==26894 & stats$precip_stats$period==precip_period,]$description, " "), "normal"), textColour(stats$precip_stats[stats$precip_stats$location_id==26894 & stats$precip_stats$period==precip_period,]$description))` from October through `r month.name[params$month-1]`
(Figure`r knitr::asis_output("\U00A0")`C). Cumulative winter
precipitation was
**`r stats$precip_stats[stats$precip_stats$location_id==26894 & stats$precip_stats$period==precip_period,]$perc_hist_med`%**
of historical median on `r month.name[params$month]` 1. Cumulative
degree-days of freezing (CDDF) are
**`r stats$cddf_stats[stats$cddf_stats$location_id==26894 & stats$cddf_stats$period==cddf_period,]$perc_hist_med`%**
of historical median, with
**`r stats$cddf_stats[stats$cddf_stats$location_id==26894 & stats$cddf_stats$period==cddf_period,]$value`$^\circ$C-Days**
on `r month.name[params$month]` 1
(Figure`r knitr::asis_output("\U00A0")`D), which suggests `r iceThickness(stats$cddf_stats[stats$cddf_stats$location_id==26894 & stats$cddf_stats$period==cddf_period,]$perc_hist_med) ` on rivers and lakes of the region.
...
`r officer::ftext(paste0(if (nchar(stats$flow_stats[stats$flow_stats$location_id=="09FD002",]$description) > 0) paste0(stats$flow_stats[stats$flow_stats$location_id=="09FD002",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$basin_stats[stats$basin_stats$basin=="Peel",]$description) > 0) paste0(stats$basin_stats[stats$basin_stats$basin=="Peel",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$flow_stats[stats$flow_stats$location_id=="10MA001",]$description) > 0) paste0(stats$flow_stats[stats$flow_stats$location_id=="10MA001",]$description, " "),
...
`r 
officer::ftext(paste0(if (nchar(stats$basin_stats[stats$basin_stats$basin=="Liard",]$description) > 0 ) paste0(stats$basin_stats[stats$basin_stats$basin=="Liard",]$description, " "),
...
# Plot SWE
if (language == "french") {
  snowbullpillow(location = station_id, custom_title = "A : Équivalent en eau de la neige à la rivière Hyland", year = year_param, month = month_param, language = "fr")
} else {
...
`r officer::ftext(paste0(if (nchar(stats$precip_stats[stats$precip_stats$location_id==32293 & stats$precip_stats$period==precip_period,]$description) > 0) paste0(stats$precip_stats[stats$precip_stats$location_id==32293 & stats$precip_stats$period==precip_period,]$description, " "), "normal"), textColour(stats$precip_stats[stats$precip_stats$location_id==32293 & stats$precip_stats$period==precip_period,]$description))` from October through `r month.name[params$month-1]`
(Figure`r knitr::asis_output("\U00A0")`C). Cumulative winter
precipitation was
**`r stats$precip_stats[stats$precip_stats$location_id==32293 & stats$precip_stats$period==precip_period,]$perc_hist_med`%**
of historical median on `r month.name[params$month]` 1. Cumulative
degree-days of freezing (CDDF) are
**`r stats$cddf_stats[stats$cddf_stats$location_id==32293 & stats$cddf_stats$period==cddf_period,]$perc_hist_med`%**
of historical median, with
**`r stats$cddf_stats[stats$cddf_stats$location_id==32293 & stats$cddf_stats$period==cddf_period,]$value`$^\circ$C-Days**
on `r month.name[params$month]` 1
(Figure`r knitr::asis_output("\U00A0")`D), which suggests `r iceThickness(stats$cddf_stats[stats$cddf_stats$location_id==32293 & stats$cddf_stats$period==cddf_period,]$perc_hist_med) ` on rivers and lakes of the region.
...
`r officer::ftext(paste0(if (nchar(stats$flow_stats[stats$flow_stats$location_id=="10AA001",]$description) > 0) paste0(stats$flow_stats[stats$flow_stats$location_id=="10AA001",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$basin_stats[stats$basin_stats$basin=="Alsek",]$description) > 0) paste0(stats$basin_stats[stats$basin_stats$basin=="Alsek",]$description, " "),
...
`r officer::ftext(paste0(if (nchar(stats$flow_stats[stats$flow_stats$location_id=="08AB001",]$description) > 0) paste0(stats$flow_stats[stats$flow_stats$location_id=="08AB001",]$description, " "),
...
# Find which locations are compound
cmpd <- DBI::dbGetQuery(con, "SELECT l.location 
                      FROM locations l
                      JOIN sample_series ss ON l.location_id = ss.location_id
                      WHERE ss.source_fx_args LIKE '%old_loc%' 
                      AND ss.source_fx = 'downloadSnowCourse'")
...
# Add C next to compound locations
if (nrow(cmpd) > 0) {
  tabl[tabl$location_id %in% cmpd$location, "years"] <- paste0(tabl[tabl$location_id %in% cmpd$location, "years"], " C")
  tabl[tabl$location_id %in% cmpd$location, "swe_med"] <- paste0(tabl[tabl$location_id %in% cmpd$location, "swe_med"], " C")
...
* Whitehorse Airport combines records from the old Whitehorse Airport snow course located within the airport grounds with new measurements taken a few hundred meters to the West of the airport. Two years of overlapping records were used to adjust the historical station data (1965-2022) to the new location.
* Twin Creeks B combines records from  old Twin Creeks A snow course located at the West end of the Twin Creeks air strip with new measurements taken at the East end of the air strip. Five years of overlapping records are currently used to adjust the historical station data (1977-2015) to the new location.
* Hyland River B combines records from the old Hyland River snow course located near the Hyland River air strip with new measurements taken 6 kilometers to the north of the old station. Five years of overlapping records were used to adjust the historical station data (1976-2017) to the new location.
"
} else {
"
...
* L'aéroport de Whitehorse combine les enregistrements de l'ancien parcours nivométrique de l'aéroport de Whitehorse situé dans l'enceinte de l'aéroport avec les nouvelles mesures prises à quelques centaines de mètres à l'ouest de l'aéroport. Deux années d'enregistrements chevauchants ont été utilisées pour ajuster les données de l'ancienne station (1965-2022) à l'emplacement de la nouvelle station.
* Twin Creeks B combine les enregistrements de l'ancien parcours nivométrique de Twin Creeks A situé à l'extrémité ouest de la piste d'atterrissage de Twin Creeks avec les nouvelles mesures prises à l'extrémité est de la piste d'atterrissage. Cinq années d'enregistrements chevauchants sont actuellement utilisées pour ajuster les données de l'ancienne station (1977-2015) à l'emplacement de la nouvelle station.
* Hyland River B combine les enregistrements de l'ancien parcours nivométrique de la rivière Hyland situé près la piste d'atterrissage de la rivière Hyland avec les nouvelles mesures prises à 6 kilomètres au nord de l'ancienne station. Cinq années d'enregistrements chevauchants ont été utilisées pour ajuster les données de l'ancienne station (1976-2017) à l'emplacement de la nouvelle station.
"
}
`
