...
`r officer::ftext(paste0(stats$precip_stats[stats$precip_stats$location_id == 26894 & stats$precip_stats$period == precip_period,]$description, if (language == "english") "normal" else "la normale"), textColour(stats$precip_stats[stats$precip_stats$location_id == 26894 & stats$precip_stats$period == precip_period,]$description))` from October through `r month_names[params$month-1]`
`r if (language == "english") {
...
`r officer::ftext(paste0(stats$precip_stats[stats$precip_stats$location_id == 32293 & stats$precip_stats$period == precip_period,]$description, if (language == "english") "normal" else "la normale"), textColour(stats$precip_stats[stats$precip_stats$location_id == 32293 & stats$precip_stats$period == precip_period,]$description))` from October through `r month_names[params$month-1]`
`r if (language == "english") {
...
`r if (language == "english") "The estimated Liard River discharge at Upper Liard is currently" else "On estime que le débit de la rivière Liard à Upper Liard est"`
...
whitehorse_corrections <- strsplit(gsub(".*([0-9]\\.[0-9]{4}).*([0-9]\\.[0-9]{4}).*", "\\1 \\2", whitehorse_correction_note$note), " ")
...
twin_corrections <- strsplit(gsub(".*([0-9]\\.[0-9]{4}).*([0-9]\\.[0-9]{4}).*", "\\1 \\2", twin_correction_note$note), " ")
...
hyland_corrections <- strsplit(gsub(".*([0-9]\\.[0-9]{4}).*([0-9]\\.[0-9]{4}).*", "\\1 \\2", hyland_correction_note$note), " ")
...
- Whitehorse Airport combines records from the old Whitehorse Airport snow course located within the airport grounds with new measurements taken a few hundred meters to the West of the airport. ", whitehorse_correction_pts, " overlapping data points were used to adjust the historical station data (1965-2022) to the new location with correction factors of ", whitehorse_corrections[1], " and ", whitehorse_corrections[2], " applied to the historical SWE and depth data respectively.
- Twin Creeks B combines records from  old Twin Creeks A snow course located at the West end of the Twin Creeks air strip with new measurements taken at the East end of the air strip. ", twin_correction_pts, " overlapping data points were used to adjust the historical station data (1977-2015) to the new location with correction factors of ", twin_correction[1], " and ", twin_correction[2], " applied to the historical SWE and depth data respectively.
- Hyland River B combines records from the old Hyland River snow course located near the Hyland River air strip with new measurements taken 6 kilometers to the north of the old station. ", twin_correction_pts, " overlapping data points were used to adjust the historical station data (1976-2017) to the new location with correction factors of ", hyland_corrections[1], " and ", hyland_corrections[2], " applied to the historical SWE and depth data respectively.
")
} else {
paste0("
...
- L'aéroport de Whitehorse combine les enregistrements de l'ancien parcours nivométrique de l'aéroport de Whitehorse situé dans l'enceinte de l'aéroport avec les nouvelles mesures prises à quelques centaines de mètres à l'ouest de l'aéroport. Deux années d'enregistrements chevauchants ont été utilisées pour ajuster les données de l'ancienne station (1965-2022) à l'emplacement de la nouvelle station avec des facteurs de correction de ", whitehorse_corrections[1], " et ", whitehorse_corrections[2], " appliqués aux données historiques de l'EEN et de l'épaisseur respectivement.
- Twin Creeks B combine les enregistrements de l'ancien parcours nivométrique de Twin Creeks A situé à l'extrémité ouest de la piste d'atterrissage de Twin Creeks avec les nouvelles mesures prises à l'extrémité est de la piste d'atterrissage. Cinq années d'enregistrements chevauchants sont actuellement utilisées pour ajuster les données de l'ancienne station (1977-2015) à l'emplacement de la nouvelle station avec des facteurs de correction de ", twin_correction[1], " et ", twin_correction[2], " appliqués aux données historiques de l'EEN et de l'épaisseur respectivement.
- Hyland River B combine les enregistrements de l'ancien parcours nivométrique de la rivière Hyland situé près la piste d'atterrissage de la rivière Hyland avec les nouvelles mesures prises à 6 kilomètres au nord de l'ancienne station. Cinq années d'enregistrements chevauchants ont été utilisées pour ajuster les données de l'ancienne station (1976-2017) à l'emplacement de la nouvelle station avec des facteurs de correction de ", hyland_corrections[1], " et ", hyland_corrections[2], " appliqués aux données historiques de l'EEN et de l'épaisseur respectivement.
")
}
`
