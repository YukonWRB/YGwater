...
```{r intro, eval=FALSE, include=FALSE}
...
```{r intro text, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
if (report_name %in% c("Southern Lakes", "Southern lakes", "southern lakes")){
  cat("  \nWater levels in the Southern Lakes are driven by a combination of snowmelt, summer precipitation and glacier melt. Weather conditions through the summer will determine the peak water level in the Southern Lakes, which typically occurs in late summer in response to peak glacial runoff and precipitation events.  \n  \nThis report provides an overview of current conditions.")
} else {
  cat("  \nThis report provides an overview of current conditions at selected stations across the territory. Advisories are issued for regions experiencing high or rapidly rising water levels and flows.")
}
```
...
if (report_type %in% c("Both", "Level", "both", "level")){
  ...
  for (i in stations) {
    ...
    tbl <- data.frame(
      Station_name = if (!is.na(tidyhydat::hy_stations(i)$STATION_NUMBER[1])) {stringr::str_to_title(tidyhydat::hy_stations(i)$STATION_NAME)} else{paste0(i, " DOES NOT EXIST")},
      ID = i,
      Start_year = NA,
    ...
    tryCatch ({
      #Get the level data
      if (!is.null(extra_years) & i %in% years$station) {
      ...
      if (datum_na) { #then there is no datum elevation
      ...
      } else { #there is a datum elevation
        startYearLevel <- min(lubridate::year(levelData[[1]]$Date), na.rm = T)
        todayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date) - 60*60), to = as.POSIXct(max(levelData[[3]]$Date)), by = "5 min"))$Level, na.rm=TRUE)
...
if (report_type %in% c("Both", "Flow", "both", "flow")){
  ...
  for(i in stations) {
    ...
    if (!stringr::str_detect(station_info$STATION_NAME, "LAKE") | stringr::str_detect(station_info$STATION_NAME, "RIVER")){ #Lakes don't have flow measurements, but sometimes LAKE is mentioned when describing the river location
...
if (force_CGVD28){
  stationInfo <- lapply(stations, function(x) dplyr::bind_cols(tidyhydat::hy_stations(x)[c(2,1,7,8)], if(nrow(dplyr::slice_tail(as.data.frame(tidyhydat::hy_stn_datum_conv(x)[,3])))==1) as.data.frame(tidyhydat::hy_stn_datum_conv(x)[1,3]) else (data.frame(DATUM_TO="Local datum")))) %>%
...
names(stationInfo)[names(stationInfo) == "LATITUDE"] <- "Latitude"
names(stationInfo)[names(stationInfo) == "LONGITUDE"] <- "Longitude"
names(stationInfo)[names(stationInfo) == "DATUM_TO"] <- "Datum"
...
```{r Rates table for levels, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

if (rate & report_type %in% c("Both", "Level", "both", "level")){
  
  cat("  \n# Summary of water levels and trajectories")
  
  if (report_name %in% c("Southern Lakes", "Southern lakes", "southern lakes")){
    info <- dplyr::bind_rows(levelInfo)[c(2, 5, 6, 8, 10)]
    # Populate vectors with distance to 2021 and 2007 peaks    
    distpeak2007 <- vector()
    distpeak2021 <- vector()
    for (i in stations){
    ...
    if (!is.null(level_returns)){
      info <- dplyr::bind_rows(levelInfo)[c(2, 5, 6, 8, 10, 12, 19)]
    } else {
      info <- dplyr::bind_rows(levelInfo)[c(2, 5, 6, 8, 10, 12)]
    }
  }
  
  for (i in 1:nrow(info)){
    info$Trajectory[i] <- paste0(ifelse(info$X24_hr_change_cm[i] > 0, "Rising", "Dropping"), " at ", ifelse(abs(info$X24_hr_change_cm[i]) > abs(info$X24_hr_change_cm[i] - info$X48_hr_change_cm[i]), "increasing", "decreasing"), " rate")
  }
  
  
  if (report_name %in% c("Southern Lakes", "Southern lakes", "southern lakes")){
    rateTable <- dplyr::bind_cols(stationInfo[,1], info[, c(2:4, 6)])
    rateTable$X24_hr_change_cm <- round(rateTable$X24_hr_change_cm, 1)
    rateTable$`Difference from 2007 peak (cm)` <- distpeak2007
    rateTable$`Difference from 2021 peak (cm)` <- distpeak2021
    colnames(rateTable) <- c("Station name (number)", "Current level (m)", "Current level (masl)*", "24 hr change (cm)", "Trajectory", "Difference from 2007 peak (cm)", "Difference from 2021 peak (cm)")
  } else {
    rateTable <- dplyr::bind_cols(stationInfo[,1], info[, c(2:4, 6:8)])
    rateTable$X24_hr_change_cm <- round(rateTable$X24_hr_change_cm, 1)
    rateTable$X72_hr_change_cm <- round(rateTable$X72_hr_change_cm, 1)
    colnames(rateTable) <- c("Station name (number)", "Current level (m)", "Current level (masl)*", "24 hr change (cm)", "72 hr change (cm)", "Current return period", "Trajectory")
  }
  
  print(knitr::kable(rateTable, digits=2, align = "lccccccc"))
  
  if (force_CGVD28){
    cat("*masl = metres above sea level  \n  \n**Notes:**  \nCurrent water levels are presented relative to the local datum (column 2) and relative to the Geodetic Survey of Canada Datum, which is assumed by Water Survey of Canada to refer to Canadian Geodetic Vertical Datum 1928 (CGVD28, column 3). The local datum is an arbitrary elevation used at each hydrometric station and reported by the Water Survey of Canada at [wateroffice.ec.gc.ca](https://wateroffice.ec.gc.ca/). Local datum values can be compared at one station across years but not across stations, while masl elevations can be compared across stations and years with consideration of the datum used. More recent datums are published for each of these stations on the Water Survey of Canada website. This report uses CGVD28 to match elevations reported by the Yukon Energy Corporation for Marsh Lake.")
  } else {
    cat("*masl = metres above sea level  \n  \n**Notes:**  \nCurrent water levels are presented relative to the local datum (column 2) and relative to the Canadian Geodetic Vertical Datum 2013 (CGVD2013, column 3). The local datum is an arbitrary elevation used at each hydrometric station and reported by the Water Survey of Canada at [wateroffice.ec.gc.ca](https://wateroffice.ec.gc.ca/). Local datum values can be compared at one station across years but not across stations, while masl elevations can be compared across stations and years with consideration to the datum used. Older (deprecated) vertical datums are published for each station on the Water Survey of Canada website. Datum information is provided in a table of station information at the end of this document.")
  }
}
...
if (report_type %in% c("Both", "Level", "both", "level") & length(levelInfo) > 0){
  ...
  if (!rate){
    ...
    if (!level_returns %in% c("none", "None")){
      ...
      levelTable <- dplyr::bind_rows(levelInfo)[c(5, 6, 8, 12)]
    }
    ...
    if (level_returns %in% c("none", "None")){
      colnames(levelTable) <- c("Station name (number)", "Current level (m)", "Current level (masl*)", "24 hr change (cm)", "72 hr change (cm)")
    } else {
    ...
    print(knitr::kable(levelTable, digits=2, align="llcccccc"))
...
if (report_type %in% c("Both", "Flow", "both", "flow") & length(flowInfo) > 0){
  ...
  if (!flow_returns %in% c("none", "None")){
    flowTable <- dplyr::filter(flowTable, Current_flow != 0.0001)[c(20,1, 5, 6, 8, 12, 17, 18)] #0.0001 is the indicator for lake stations, remove those
  } else {
    flowTable <- dplyr::filter(flowTable, Current_flow != 0.0001)[c(20, 1, 5, 6, 8, 12, 17)] #0.0001 is the indicator for lake stations, remove those
  }
  ...
  if (!flow_returns %in% c("none", "None")){
    colnames(flowTable) <- c("Community", "Station name (number)", "Current flow (m^3^/s)", "Current percent of historic max", "24 hr change (m^3^/s)", "72 hr change (m^3^/s)", "One week change (m^3^/s)", "Current return")
  } else {
  ...
  print(knitr::kable(flowTable, digits=1, align="c"))
  ...
  if (TRUE %in% stringr::str_detect(flowTable$`Current return period`, "\\*")){
    cat("*Value obtained programatically by analysing May to September daily mean flows. For informational purposes only: no human checks were performed on underlying data.")
  }
}
```
...
```{r Get MESH and CLEVER information, echo=FALSE, results='asis', message=FALSE}
...
if (MESH){
  ...
  tryCatch({ #tryCatch in case there is no MESH to get.
    ...
    names(MESH)[names(MESH) == "Marsh"] <- "09AB004"
  }, error = function(e) {
    MESH <- list()
  })
} else { MESH <- list()}
...
for(i in stations) {
  n <- which(stations==i)
  station_info <- tidyhydat::hy_stations(i)
  if (!is.na(station_info$STATION_NUMBER[1])) { #if the station is in the database, continue!
    tryCatch ({
    ...
    cat("  \n Station ", i, " could not be found in the WSC HYDAT database.\n")
    cat("  \n\\newpage\n\n")
    
  }
} #End of For loop generating plots at each station.
...
if (precip){
  cat("# Accumulated Precipitation Above Stations  \n")
  tryCatch({
  ...
  tryCatch({
    ...
    if (nrow(images)>=1){
      ...
      for (i in unique(images$Station)){
        ...
        curl::curl_download(paste0("https://collaboration.cmc.ec.gc.ca/cmc/hydrometric_additionalData/FieldData/YT/", last_image[,1]), temp, handle = curl::new_handle(userpwd = paste(Sys.getenv("ECCCUSER"), Sys.getenv("ECCCPASS"), sep = ":")))
...
Hydrometric stations are maintained by the Water Survey of Canada and funded in partnership with the Government of Yukon. Additional station information, including on vertical datums can be found at [wateroffice.ec.gc.ca](https://wateroffice.ec.gc.ca). To learn more about hydrometric data in general please see the [Water Survey of Canada FAQ page](https://wateroffice.ec.gc.ca/contactus/faq_e.html).
...
if (report_name %in% c("Southern Lakes", "Southern lakes", "southern lakes")){
  cat("  \nThe stations in this report have more recent datums available (CGVD2013), but the historical CGVD28 datum is used here to align with local knowledge and with other reporting.")
}
```
