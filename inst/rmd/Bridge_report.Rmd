...
for (i in 1:nrow(tsid)) {
  bridge_daily[[tsid$location[i]]] <- DBI::dbGetQuery(con, paste0("SELECT value, date FROM measurements_calculated_daily WHERE timeseries_id = '", tsid$timeseries_id[i], "' AND date BETWEEN '", Sys.Date()-365, "' AND '", Sys.Date(), "'" ))
