...
for (i in 1:nrow(tsid)){
  ...
  cat("  \n### Distance (past year)  \n")
  plot <- hydrometContinuous(location = tsid[i, "location"], parameter = "distance", returns = "none", datum = FALSE, filter = 10, startDay = Sys.Date()-364, endDay = Sys.Date())
  ...
  if (zoom){
    ...
    zoom_plot <- hydrometContinuous(location = tsid[i, "location"], parameter = "distance", returns = "none", startDay = Sys.Date()-zoom_days, endDay = Sys.Date(), datum = FALSE, filter = 10)
