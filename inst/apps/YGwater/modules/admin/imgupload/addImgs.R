    ...
    mainPanel(
      ...
      fluidRow(
        column(
          ...
          selectizeInput(ns("visit_type"), "Visit type:", choices = visit_list, 
                         multiple = FALSE, width = "100%", options = list(create = FALSE)),

          selectizeInput(ns("image_tags"), "Tag(s):", multiple = TRUE, 
...
addImgs <- function(id) {
    ...
    if (!check$can_insert) {
      ...
      tag_lists <- list(`Flight`=list("Head", "Toe", "Smooth ice", "In flood"), 
    ...
    observeEvent(input$image_location, ignoreNULL = TRUE, {
      updateSelectizeInput(session, "image_sublocation", 
        choices = stats::setNames(sublocations[sublocations$location_id == input$image_location, "sub_location_id"], 
