...
addImgSeries <- function(id) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    getModuleData <- function() {
      moduleData$image_series <- DBI::dbGetQuery(session$userData$AquaCache, "SELECT img_series_id, source_fx, source_fx_args, description, location_id, active, share_with, owner FROM files.image_series;")
      moduleData$image_series_display <- DBI::dbGetQuery(session$userData$AquaCache, "SELECT i.img_series_id, l.name AS location, o.name AS owner, i.source_fx, i.active
      ...
      moduleData$locations <- DBI::dbGetQuery(session$userData$AquaCache, "SELECT location_id, location, name, latitude, longitude FROM locations")
      moduleData$users <- DBI::dbGetQuery(session$userData$AquaCache, "SELECT * FROM public.get_shareable_principals_for('files.images');")  # This is a helper function run with SECURITY DEFINER and created by postgres that pulls all user groups (plus public_reader) with select privileges on a table
    }
    ...
    output$ui <- renderUI({
      tagList(
        ...
        textAreaInput(ns("description"), "Description (optional)", 
                      value = "", 
                      rows = 3, 
        ...
        conditionalPanel(
          condition = "input.mode == 'add'",
          ns = ns,
          dateInput(ns("start_date"), "Start date to search for images", 
                    value = Sys.Date() - 30,
                    format = "yyyy-mm-dd",
                    width = "100%"),
        ...
        conditionalPanel(
          ...
          bslib::input_task_button(ns("modify_series"), label = "Modify image series")
        )
      )
    })
    ...
    observEvent(input$series_table_rows_selected, {
      ...
      if (length(sel) > 0) {
        ...
        details <- moduleData$image_series[moduleData$image_series == selected_series(), ]
        ...
        updateTextInput(session, "source_fx_args",
                        value = details$source_fx_args)
        updateCheckboxInput(session, "active",
    ...
    observeEvent(input$owner, {
      if (input$owner %in% moduleData$organizations$organization_id || nchar(input$owner) == 0) {
        return()
      }
    ...
    observeEvent(input$add_owner, {
      ...
      updateSelectizeInput(session, "owner", choices = stats::setNames(moduleData$organizations$organization_id, moduleData$organizations$name), selected = moduleData$organizations[moduleData$organizations$name == df$name, "organization_id"])
      removeModal()
      showModal(modalDialog(
        "New owner added.",
        easyClose = TRUE
    ...
    addNewSeries <- ExtendedTask$new(function(config, loc, share_with, owner, description, source_fx, source_fx_args, start){
      promises::future_promise({
        tryCatch({
          ...
          new_id <- DBI::dbGetQuery(con, "SELECT MAX(img_series_id) AS id FROM files.image_series;")$id
    ...
    observeEvent(input$add_series, {
      ...
      validate(need(input$location, "Please select a location"),
               ...
               need(input$share_with, "Please select at least one group to share with"),
               need(input$source_fx, "Please select a source function"),
               need(!isTruthy(input$source_fx_args) || grepl(":", input$source_fx_args), "Source function arguments must be formatted as key-value pairs, e.g. 'arg1: value1, arg2: value2'")
      )
    ...
    observeEvent(input$modify_series, {
      if (input$mode != "modify") {
        # This is an error: show the user a notification to select 'modify' mode
        showNotification("Please select 'Modify existing' mode to modify a series", type = "error")
        return()
      }
      ...
      selected_series <- moduleData$image_series[moduleData$image_series$img_series_id == id, ]
      # Check if the series already exists
      existing_timeseries <- DBI::dbGetQuery(session$userData$AquaCache, paste0("SELECT * FROM image_series WHERE img_series_id = ", selected_series$img_series_id))
      if (nrow(existing_timeseries) == 0) {
        showNotification("Selected image_series does not exist in the database.", type = "error")
        return()
      }
      ...
      tryCatch({
        if (input$location != selected_series$location_id) {
          DBI::dbExecute(session$userData$AquaCache, "UPDATE files.image_series SET location_id = $1 WHERE img_series_id = $2;", params = list(input$location, selected_series$img_series_id))
        }
        
        if (input$owner != selected_series$owner) {
          DBI::dbExecute(session$userData$AquaCache, "UPDATE files.image_series SET owner = $1 WHERE img_series_id = $2;", params = list(input$owner, selected_series$img_series_id))
        }
        
        if (input$description != selected_series$description) {
          DBI::dbExecute(session$userData$AquaCache, "UPDATE files.image_series SET description = $1 WHERE img_series_id = $2;", params = list(if (isTruthy(input$description)) input$description else NA, selected_series$img_series_id))
        }
        
        # Changes to share_with
        if (!paste0("{", paste(input$share_with, collapse = ","), "}") == selected_series$share_with) {
        ...
        if (input$source_fx != selected_series$source_fx) {
          DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE image_series SET source_fx = '", input$source_fx, "' WHERE img_series_id = ", selected_series$img_series_id))
        }
        
        if (!is.na(selected_series$source_fx_args)) {
          if (!nzchar(input$source_fx_args)) {
            DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE timeseries SET source_fx_args = NULL WHERE img_series_id = ", selected_series$img_series_id))
            return()
          }
          ...
          parsed_json <- jsonlite::fromJSON(selected_series$source_fx_args, simplifyVector = TRUE, flatten = TRUE)
          ...
          if (!nzchar(input$source_fx_args)) {
            DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE image_series SET source_fx_args = NULL WHERE img_series_id = ", selected_series$img_series_id))
            return()
          }
          # Make the source_fx_args a json object
          args <- input$source_fx_args
        ...
        showNotification("Image series updated successfully!", type = "message")
        getModuleData()
      }, error = function(e) {
        DBI::dbRollback(session$userData$AquaCache)
        showNotification(paste("Error updating image series:", e$message), type = "error")
      })
    }, ignoreInit = TRUE)
