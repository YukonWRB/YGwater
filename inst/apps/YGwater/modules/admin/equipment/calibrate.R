...
table.on("click", "tr", function() {
  ...
  if($row.hasClass("selected")) {
    ...
    restarted <- reactiveValues(initialized = FALSE,
                                restarted = FALSE)
    ...
    complete <- reactiveValues(basic = FALSE,
                               temperature = FALSE,
                               spc = FALSE,
                               ph = FALSE,
                               orp = FALSE,
    ...
    instruments_data$observers <- DBI::dbGetQuery(session$userData$AquaCache, "SELECT * FROM observers")
    instruments_data$makes <- DBI::dbGetQuery(session$userData$AquaCache, "SELECT * FROM instrument_make")
    instruments_data$models <- DBI::dbGetQuery(session$userData$AquaCache, "SELECT * FROM instrument_model")
    instruments_data$types <- DBI::dbGetQuery(session$userData$AquaCache, "SELECT * FROM instrument_type")
    ...
    sensors_data$sensors <- DBI::dbGetQuery(session$userData$AquaCache, paste0("SELECT * FROM sensors"))
    sensors_data$sensor_types <- DBI::dbGetQuery(session$userData$AquaCache, paste0("SELECT * FROM sensor_types"))
    ...
    ## Add new observers  ################################################
    observer_inputs <- c("observer", "recorder", "retired_by", "sensor_change_name", "add_sensor_name")
    ...
    observeEvent(input$add_new_observer, {
      ...
      DBI::dbExecute(session$userData$AquaCache, paste0("INSERT INTO observers (observer_first, observer_last, organization) VALUES ('", input$new_observer_first, "', '", input$new_observer_last, "', '", input$new_observer_org, "')"))
    ...
    observeEvent(input$add_new_sensor_type, {
      ...
      if (input$new_sensor_desc == "") {
        DBI::dbExecute(session$userData$AquaCache, paste0("INSERT INTO sensor_types (sensor_type) VALUES ('", input$new_sensor_type, "')"))
      } else {
        DBI::dbExecute(session$userData$AquaCache, paste0("INSERT INTO sensor_types (sensor_type, sensor_type_description) VALUES ('", input$new_sensor_type, "', '", input$new_sensor_desc, "')"))
      }
      sensors_data$sensor_types <- DBI::dbGetQuery(session$userData$AquaCache, paste0("SELECT * FROM sensor_types"))
      tmp <- setNames(c(sensors_data$sensor_types$sensor_type_id, "new"), c(sensors_data$sensor_types$sensor_type, "Add new type"))
    ...
    observeEvent(input$selection, {
      if (input$selection != "Basic calibration info" & !complete$basic) {
        alert(title = "Stop! You must save basic info first or load an incomplete calibration!", type = "error", timer = 2000)
    ...
    reset_temp <- function() {
      ...
      updateNumericInput(session, "temp_reference", label = "Reference Temp", value = NA)
      updateNumericInput(session, "temp_observed", label = "Sensor Temp", value = NA)
      shinyjs::hide("delete_temp")
    }
    reset_orp <- function() {
      updateNumericInput(session, "orp_std", label = "ORP Standard solution mV", value = NA)
      updateNumericInput(session, "orp_pre_mv", label = "ORP mV Pre-Cal Value", value = NA)
      updateNumericInput(session, "orp_post_mv", label = "ORP mV Post-Cal Value", value = NA)
      shinyjs::hide("delete_orp")
    }
    reset_spc <- function() {
      updateNumericInput(session, "spc1_std", label = "SpC Low-Range Standard", value = "0")
      updateNumericInput(session, "spc1_pre", label = "SpC Low-Range Pre-Cal Value", value = NA)
      ...
      updateNumericInput(session, "spc2_pre", label = "SpC High-Range Pre-Cal Value", value = NA)
    ...
    reset_turb <- function() {
      ...
      updateNumericInput(session, "turb1_pre", label = "Low Turb Pre-cal Value", value = NA)
      updateNumericInput(session, "turb2_pre", label = "High Turb Pre-cal Value", value = NA)
    ...
    reset_do <- function() {
      updateNumericInput(session, "baro_press_pre", label = "Baro Pressure Pre-Cal", value = NA)
      updateNumericInput(session, "baro_press_post", label = "Baro Pressure Post-Cal", value = NA)
      updateNumericInput(session, "do_pre_prct", label = "DO Pre-Cal %", value = NA)
      updateNumericInput(session, "do_post_prct", label = "DO Post-Cal %", value = NA)
      updateNumericInput(session, "do_pre", label = "DO Pre-Cal mg/L", value = NA)
      updateNumericInput(session, "do_post", label = "DO Post-Cal mg/L", value = NA)
      shinyjs::hide("delete_do")
    }
    ...
    observeEvent(input$load_sensors, {
      if ((input$load_sensors %% 2) == 0) { # this part runs on second and subsequent even numbered clicks
        ...
        if (input$maintain_serial != "loading choices...") {
          ...
          sensors_data$instrument <- DBI::dbGetQuery(session$userData$AquaCache, paste0("SELECT * FROM array_maintenance_changes WHERE instrument_id = ", sensors_data$instrument_id))
    ...
    observeEvent(input$add_sensor_to_db, { # adding sensor via left hand side menu
      ...
      sensors_data$sensors <- DBI::dbGetQuery(session$userData$AquaCache, "SELECT * FROM sensors")
    }, ignoreInit = TRUE, ignoreNULL = TRUE)
    ...
    observeEvent(input$add_sensor_slot, {
      ...
      if (sensors_data$datetime_exists) { #modifying an existing entry
        DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE array_maintenance_changes SET ", col_id, " = ", sensor_id, ", ", col_comment, " = '", comment, "' WHERE obs_datetime BETWEEN '", sensors_data$datetime - 5, "' AND '", sensors_data$datetime + 5, "' AND instrument_id = ", sensors_data$instrument_id, ";"))
        ...
        DBI::dbAppendTable(session$userData$AquaCache, "array_maintenance_changes", df)
      }
    ...
    observeEvent(input$submit_sensor_change, {
      if (nchar(input$sensor_change_name) <= 1 | nchar(input$add_comment) < 5 | nchar(input$add_sensor_serial) < 2) {
        ...
        if (sensors_data$datetime_exists) {  # Update the existing row
          DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE array_maintenance_changes SET ", col_id, " = ", sensor_id, ", ", col_comment, " = '", input$add_comment, "', WHERE obs_datetime = '", sensors_data$datetime, "' AND instrument_id = ", sensors_data$instrument_id, ";"))
          run_else <- FALSE
    ...
    observeEvent(input$save_cal_instrument, { #save the new record or the changes to existing record
      ...
      if (input$existing_serial_no == "New record") { #add a new row with the next instrument_id
        if (check) {
          ...
          DBI::dbAppendTable(session$userData$AquaCache, "instruments", instrument.df)
          
          alert(paste0("Serial number ", input$serial_no, " added"), type = "success", timer = 2000)
        }
        ...
        if (nchar(input$recorder) < 1 | nchar(input$make) < 1 | nchar(input$model) < 1 | nchar(input$type) < 1 | (nchar(input$serial_no) < 4 | nchar(input$instrument_owner) < 1 | (grepl("Search", input$serial_no, ignore.case = TRUE)))) { #long statement to check if all entries are satisfactorily filled in.
          ...
          if (nchar(input$asset_tag) > 0) {
            DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE instruments SET asset_tag = '", gsub("[^[:alnum:]]", "", input$asset_tag), "' WHERE serial_no = '", input$existing_serial_no, "'"))
          }
          if (length(input$date_in_service) > 0) {
            DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE instruments SET date_in_service = '", input$date_in_service, "' WHERE serial_no = '", input$existing_serial_no, "'"))
          }
          if (length(input$date_purchased) > 0) {
            DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE instruments SET date_purchased = '", input$date_purchased, "' WHERE serial_no = '", input$existing_serial_no, "'"))
          }
          if (nchar(input$retired_by) > 0) {
            DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE instruments SET retired_by = '", input$retired_by, "' WHERE serial_no = '", input$existing_serial_no, "'"))
          }
          if (length(input$date_retired) > 0) {
            DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE instruments SET date_retired = '", input$date_retired, "' WHERE serial_no = '", input$existing_serial_no, "'"))
          }
          
          alert(paste0("Serial number ", input$existing_serial_no, " modified"), type = "success", timer = 2000)
          new_entry <- TRUE
        }
      }
    ...
    observeEvent(input$restart_calibration, {
      ...
      if (restart_value == 0) {
        ...
        for (i in calibration_sheets) {
          sheet <- DBI::dbGetQuery(session$userData$AquaCache, paste0("SELECT * FROM ", i, " WHERE calibration_id = ", incomplete_ID))
          if (nrow(sheet) == 1) {
    ...
    observeEvent(input$delete_calibration, {
      ...
      if (delete_value == 0) {
        ...
        DBI::dbExecute(session$userData$AquaCache, paste0("DELETE FROM calibrations WHERE calibration_id = ", delete_ID))  # Cascades to all other sheets where the id is referenced
    ...
    DO_calc <- function(pre_post, prct_abs, messages = TRUE) {
      ...
      if (!is.na(baro_press)) {
        if (baro_press < 600 | baro_press > 850) {
          if (pre_post == "pre") {
            alert("Pre-calibration baro pressure out of range", "Enter pressure in mmHg only", type = "error", timer = 3000)
          } else {
            alert("Post-calibration baro pressure out of range", "Enter pressure in mmHg only", type = "error", timer = 3000)
          }
        } else {
          go_baro <- TRUE
        }
        ...
        if (pre_post == "pre") {
          alert("Enter pre-cal baro pressures in mmHg first!", type = "error", timer = 3000)
        } else {
          alert("Enter post-cal baro pressures in mmHg first!", type = "error", timer = 3000)
        }
      }
      if (!is.na(temp)) {
        if (temp < 0 | temp > 30) {
          if (messages) {
            alert("Temperature out of range", "Temp should be between 0 and 30 degrees C; review temperature calibration", type = "error", timer = 3000)
          }
        } else {
          go_temp <- TRUE
        }
        ...
        if (messages) {
          alert("Enter temperature data first!", "Go to the temperature calibration", type = "error", timer = 3000)
        }
      }
    ...
    observeEvent(input$save_basic_info, {
      if (nchar(input$ID_sensor_holder) < 1) {
        alert(title = "Fill in the logger/bulkhead serial #", type = "error", timer = 2000)
        return()
      }
      ...
      if (nchar(input$observer) < 1) {
        alert(title = "Fill in the observer name", type = "error", timer = 2000)
        return()
      }
      ...
      if (!complete$basic) { # New entry
        DBI::dbAppendTable(session$userData$AquaCache, "calibrations", calibration_data$basic)
        calibration_data$next_id <- DBI::dbGetQuery(session$userData$AquaCache, "SELECT MAX(calibration_id) FROM calibrations")[[1]]
        complete$basic <- TRUE
      } else { # Modify an existing entry
        if (is.na(id_handheld_meter)) {
          DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE calibrations SET observer = '", input$observer, "', obs_datetime = '", dt, "', id_sensor_holder = ", id_sensor_holder, " WHERE calibration_id = ", calibration_data$next_id, ";"))
        } else {
          DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE calibrations SET observer = '", input$observer, "', obs_datetime = '", dt, "', id_sensor_holder = ", id_sensor_holder,  ", id_handheld_meter = ", id_handheld_meter, " WHERE calibration_id = ", calibration_data$next_id, ";"))
        }
        if (!is.na(purpose_text)) {
          DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE calibrations SET purpose = '", purpose_text, "' WHERE calibration_id = ", calibration_data$next_id, ";"))
        }
      }
    ...
    observeEvent(validation_check$ph, {
      if (!validation_check$ph) {
        ...
        if (!complete$ph) {
          DBI::dbAppendTable(session$userData$AquaCache, "calibrate_ph", calibration_data$ph)
          
          complete$ph <- TRUE
          shinyjs::show("delete_ph")
        } else {
          DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE calibrate_ph SET ph1_std = ", input$ph1_std,
                                                            ", ph2_std = ", input$ph2_std,
                                                            ", ph3_std = ", input$ph3_std,
                                                            ", ph1_pre_val = ", input$ph1_pre_val,
                                                            ", ph2_pre_val = ", input$ph2_pre_val,
    ...
    observeEvent(input$delete_ph, {
      ...
      DBI::dbExecute(session$userData$AquaCache, paste0("DELETE FROM calibrate_ph WHERE calibration_id = ", calibration_data$next_id))
      #reset the fields
      reset_ph()
    ...
    observeEvent(input$save_cal_temp, {
      ...
      tryCatch({
        ...
        alert(title = "You have unfilled mandatory entries", type = "error", timer = 2000)
      })
    }, ignoreInit = TRUE)
    ...
    observeEvent(validation_check$temp, {
      if (!validation_check$temp) {
        ...
        if (!complete$temperature) {
          DBI::dbAppendTable(session$userData$AquaCache, "calibrate_temperature", calibration_data$temp)
          complete$temperature <- TRUE
          shinyjs::show("delete_temp")
        } else {
          DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE calibrate_temperature SET temp_reference_desc = '", input$temp_reference_desc,
                                                            "', temp_reference = ", input$temp_reference,
                                                            ", temp_observed = ", input$temp_observed,
                                                            " WHERE calibration_id = ", calibration_data$next_id))
        }
    ...
    observeEvent(input$delete_temp, {
      DBI::dbExecute(session$userData$AquaCache, paste0("DELETE FROM calibrate_temperature WHERE calibration_id = ", calibration_data$next_id))
      #reset the fields
      reset_temp()
      #remove from display tables
    ...
    observeEvent(validation_check$orp, {
      if (!validation_check$orp) {
        ...
        if (!complete$orp) {
          DBI::dbAppendTable(session$userData$AquaCache, "calibrate_orp", calibration_data$orp)
          complete$orp <- TRUE
          shinyjs::show("delete_orp")
        } else {
          DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE calibrate_orp SET orp_std = ", input$orp_std,
                                                            ", orp_pre_mv = ", input$orp_pre_mv,
                                                            ", orp_post_mv = ", input$orp_post_mv,
                                                            " WHERE calibration_id = ", calibration_data$next_id))
        }
    ...
    observeEvent(input$delete_orp, {
      DBI::dbExecute(session$userData$AquaCache, paste0("DELETE FROM calibrate_orp WHERE calibration_id = ", calibration_data$next_id))
      #reset the fields
      reset_orp()
      #remove from display tables
      ...
      if (input$spc_or_not & is.null(calibration_data$temp)) {
        ...
        tryCatch({
          ...
          if (spc2_diff > 10) {
            ...
            alert(title = "Warning: double check your values", type = "warning", timer = 2000)
            if (input$spc_or_not) {
              message1 <- paste0("Double check your values: your high-range input converts to an SpC of ", round(spc2_post, 0), " versus the expected ", spc2_ref)
            } else {
              message1 <- "Warning: double check your high-range values"
      ...
      if (!validation_check$spc) {
        ...
        if (!complete$spc) {
          tryCatch({
            ...
            alert(title = "Failed to make new entry to database... are you sure all entries are correct and that you are entering specific or non-specific conductivity as required by your instrument?", type = "error", timer = 4000)
          })
        } else {
          tryCatch({
            DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE calibrate_specific_conductance SET spc1_std = ", input$spc1_std,
                                                              ", spc1_pre = ", if (input$spc_or_not) input$spc1_pre/(1 + 0.02*(calibration_data$temp$temp_reference - 25)) else input$spc1_pre,
                                                              ", spc1_post = ", if (input$spc_or_not) input$spc1_post/(1 + 0.02*(calibration_data$temp$temp_reference - 25)) else input$spc1_post,
                                                              ", spc2_std = ", input$spc2_std,
                                                              ", spc2_pre = ", if (input$spc_or_not) input$spc2_pre/(1 + 0.02*(calibration_data$temp$temp_reference - 25)) else input$spc2_pre,
    ...
    observeEvent(input$delete_spc, {
      DBI::dbExecute(session$userData$AquaCache, paste0("DELETE FROM calibrate_specific_conductance WHERE calibration_id = ", calibration_data$next_id))
      #reset the fields
      reset_spc()
      #remove from display tables
    ...
    observeEvent(validation_check$turb, {
      if (!validation_check$turb) {
        ...
        if (!complete$turbidity) {
          DBI::dbAppendTable(session$userData$AquaCache, "calibrate_turbidity", calibration_data$turb)
          complete$turbidity <- TRUE
          shinyjs::show("delete_turb")
        } else {
          DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE calibrate_turbidity SET turb1_std = ", input$turb1_std,
                                                            ", turb1_pre = ", input$turb1_pre,
                                                            ", turb1_post = ", input$turb1_post,
                                                            ", turb2_std = ", input$turb2_std,
                                                            ", turb2_pre = ", input$turb2_pre,
    ...
    observeEvent(input$delete_turb, {
      DBI::dbExecute(session$userData$AquaCache, paste0("DELETE FROM calibrate_turbidity WHERE calibration_id = ", calibration_data$next_id))
      #reset the fields
      reset_turb()
      #remove from display tables
    ...
    observeEvent(input$save_cal_do, {
      ...
      tryCatch({
        ...
        alert(title = "You have unfilled mandatory entries", "Baro pressure and DO in mg/l are mandatory", type = "error", timer = 4000)
      })
    }, ignoreInit = TRUE)
    ...
    observeEvent(validation_check$do, {
      if (!validation_check$do) {
        ...
        if (!complete$do) {
          DBI::dbAppendTable(session$userData$AquaCache, "calibrate_dissolved_oxygen", calibration_data$do)
          
          complete$do <- TRUE
          shinyjs::show("delete_do")
        } else {
          DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE calibrate_dissolved_oxygen SET baro_press_pre = ", input$baro_press_pre,
                                                            ", baro_press_post = ", input$baro_press_post,
                                                            ", do_pre_mgl = ", input$do_pre,
                                                            ", do_post_mgl = ", input$do_post,
                                                            " WHERE calibration_id = ", calibration_data$next_id))
    ...
    observeEvent(input$delete_do, {
      DBI::dbExecute(session$userData$AquaCache, paste0("DELETE FROM calibrate_dissolved_oxygen WHERE calibration_id = ", calibration_data$next_id))
      #reset the fields
      reset_do()
      #remove from display tables
    ...
    observeEvent(validation_check$depth, {
      if (!validation_check$depth) {
        ...
        if (!complete$depth) {
          DBI::dbAppendTable(session$userData$AquaCache, "calibrate_depth", calibration_data$depth)
          complete$depth <- TRUE
          shinyjs::show("delete_depth")
        } else {
          if (input$depth_changes_ok != "Not Checked") {
            DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE calibrate_depth SET depth_check_ok = '", input$depth_check_ok, "', depth_changes_ok ='", input$depth_changes_ok, "' WHERE calibration_id = ", calibration_data$next_id))
          } else {
            DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE calibrate_depth SET depth_check_ok = '", input$depth_check_ok, "', depth_changes_ok = NULL WHERE calibration_id = ", calibration_data$next_id))
          }
        }
    ...
    observeEvent(input$delete_depth, {
      DBI::dbExecute(session$userData$AquaCache, paste0("DELETE FROM calibrate_depth WHERE calibration_id = ", calibration_data$next_id))
    ...
    observeEvent({input$submit_btn}, {
      if (!("Basic info" %in% send_table$saved[ ,1] | "Basic info" %in% send_table$restarted_cal[ ,1])) {
        alert(title = "There is no basic information yet!!!", text = "Fill in your name, calibration time and date, and required serial numbers.", type = "error")
      } else if (!("Temperature calibration" %in% send_table$saved[ ,1] | "Temperature calibration" %in% send_table$restarted_cal[ ,1])) {
        alert(title = "Temperature calibration is mandatory", text = "If you've filled it in already you probably forgot to save it!", type = "error")
      } else {
        showModal(
          modalDialog(
            title = "Are you sure?",
    ...
    observeEvent(input$confirm_finalize, {
      removeModal()
      DBI::dbExecute(session$userData$AquaCache, paste0("UPDATE calibrations SET complete = 'TRUE' WHERE calibration_id = ", calibration_data$next_id))
      calibrations$calibrations <- DBI::dbReadTable(session$userData$AquaCache, "calibrations")
