manageUsersUI <- function(id) {
  ...
  page_fluid(
    tagList(
      selectizeInput(ns("group_user"), NULL, choices = stats::setNames(c("groups", "users", "users_to_groups"), c("Create new group", "Create new user", "Assign users to groups"))),
      conditionalPanel(
      ...
      conditionalPanel(
        ...
        ns = ns,
        h4("Add user to a group. They'll inherit that the group's default permissions on schemas and tables as well as see records restricted to that group. You can add users to multiple groups as needed."),
...
manageUsers <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    schemas <- DBI::dbGetQuery(session$userData$AquaCache, "
SELECT schema_name
...
WHERE schema_name NOT LIKE 'pg_%' 
  ...
  AND schema_name <> 'information';")
    ...
    load_roles <- function() {
      roles <- DBI::dbGetQuery(session$userData$AquaCache,
      ...
      if (nchar(input$group_name) > 0) {
        if (!group_options_rendered()) {
          output$group_options_ui <- renderUI({
            tagList(
              selectizeInput(ns("schema_permissions"), "Grant usage on schemas", choices = schemas$schema_name, multiple = TRUE),
              # Selectize inputs below are populated dynamically based on the selected schemas
              selectizeInput(ns("table_permission_select"), "Grant SELECT on tables", choices = NULL, multiple = TRUE),
              selectizeInput(ns("table_permission_insert"), "Grant INSERT on tables", choices = NULL, multiple = TRUE),
              selectizeInput(ns("table_permission_delete"), "Grant DELETE on tables", choices = NULL, multiple = TRUE),
              selectizeInput(ns("table_permission_update"), "Grant UPDATE on tables", choices = NULL, multiple = TRUE),
    ...
    observeEvent(input$schema_permissions, {
      ...
      tables <- DBI::dbGetQuery(session$userData$AquaCache, sprintf(
        "SELECT table_schema, table_name 
         FROM information_schema.tables 
         WHERE table_schema IN (%s)
         AND table_type = 'BASE TABLE'
        ORDER BY table_schema, table_name;",
        paste(DBI::dbQuoteString(session$userData$AquaCache, input$schema_permissions), collapse = ", ")
      ))
      table_choices <- paste0(tables$table_schema, ".", tables$table_name)
      updateSelectizeInput(session, "table_permission_select", choices = c("All tables", table_choices), selected = NULL)
      updateSelectizeInput(session, "table_permission_insert", choices = c("All tables", table_choices), selected = NULL)
      updateSelectizeInput(session, "table_permission_delete", choices = c("All tables", table_choices), selected = NULL)
      updateSelectizeInput(session, "table_permission_update", choices = c("All tables", table_choices), selected = NULL)
    })
    ...
    observeEvent(input$create_group, {
      ...
      if (length(input$schema_permissions) == 0) {
        output$status <- renderText("Please select at least one schema for the group to have usage on.")
        return(NULL)
      }
      
      if (length(input$table_permission_select) == 0 &&
          length(input$table_permission_insert) == 0 &&
          length(input$table_permission_update) == 0 &&
          length(input$table_permission_delete) == 0) {
        output$status <- renderText("Please select at least one table permission for the group.")
        return(NULL)
      }
      
      # Ensure the group doesn't already exist
      if (input$group_name %in% (DBI::dbGetQuery(session$userData$AquaCache, "SELECT rolname FROM pg_roles")$rolname)) {
        output$status <- renderText(sprintf("Group '%s' already exists", input$group_name))
        return(NULL)
      }
      ...
      tryCatch({
        ...
        sql <- paste0("CREATE ROLE ", input$group_name, " NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;")
        DBI::dbExecute(session$userData$AquaCache, sql)
        
        # Give the user group usage on relevant schemas
        if (!is.null(input$schema_permissions) && length(input$schema_permissions) > 0) {
          for (schema in input$schema_permissions) {
            sql <- paste0(sprintf(
              "GRANT USAGE ON SCHEMA %s TO %s;",
              DBI::dbQuoteIdentifier(session$userData$AquaCache, schema),
              DBI::dbQuoteIdentifier(session$userData$AquaCache, input$group_name)
            ))
            DBI::dbExecute(session$userData$AquaCache, sql)
          }
        }
        ...
        if (!is.null(input$table_permission_select) && length(input$table_permission_select) > 0) {
          grant_table_permissions(input$table_permission_select, "SELECT")
        }
        if (!is.null(input$table_permission_insert) && length(input$table_permission_insert) > 0) {
          grant_table_permissions(input$table_permission_insert, "INSERT")
        }
        if (!is.null(input$table_permission_update) && length(input$table_permission_update) > 0) {
          grant_table_permissions(input$table_permission_update, "UPDATE")
        }
        if (!is.null(input$table_permission_delete) && length(input$table_permission_delete) > 0) {
    ...
    observeEvent(input$create_user, {
      ...
      tryCatch({
        ...
        DBI::dbExecute(session$userData$AquaCache, sql)
        load_roles()
        output$status <- renderText(sprintf("Created user '%s'. Remember to add them to relevant user groups!", input$user_name))
      }, error = function(e) {
        output$status <- renderText(e$message)
      })
    })
    ...
    observeEvent(input$add_user_group, {
      ...
      tryCatch({
        sql <- sprintf(
          "GRANT %s TO %s;",
          DBI::dbQuoteIdentifier(session$userData$AquaCache, input$existing_group),
          DBI::dbQuoteIdentifier(session$userData$AquaCache, input$existing_user)
        )
        DBI::dbExecute(session$userData$AquaCache, sql)
