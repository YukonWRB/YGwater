  ...
  moduleServer(id, function(input, output, session) {
    ...
    loadMappings <- function(con) {
      if (
        !DBI::dbExistsTable(
          con,
          DBI::Id(schema = "application", table = "discrete_mappings")
        )
      ) {
        return(list())
      }
      df <- DBI::dbGetQuery(
        con,
        "SELECT mapping_name, mapping FROM application.discrete_mappings"
      )
    ...
    observe({
      updateSelectizeInput(
        session,
        "mapping_select",
        choices = c("", names(mappings()))
      )
    })

    observeEvent(
      input$mapping_select,
      {
    ...
    output$long_format_cols <- renderUI({
      ...
      tagList(
        selectizeInput(
          ...
          "Parameter column",
          choices = names(df),
          selected = current$param_col
        ),
        selectizeInput(
          ns("long_unit_col"),
          "Unit column",
          choices = c("", names(df)),
          selected = current$unit_col
        ),
        selectizeInput(
          ns("long_value_col"),
          "Value column",
          choices = names(df),
          selected = current$value_col
        )
      )
    })
    ...
    output$mapping_ui <- renderUI({
      ...
      if (is.null(current$format)) {
        current$format <- input$file_format
      }
      if (identical(current$format, "long")) {
        ...
        tagList(
          ...
          lapply(param_names, function(p) {
            ...
            par_sel <- if (!is.null(current$params[[as.character(p)]])) {
              current$params[[as.character(p)]]$id
            } else {
              NULL
            }
            conv_sel <- if (!is.null(current$params[[as.character(p)]])) {
              current$params[[as.character(p)]]$conv
            } else {
              1
            }
            fluidRow(
        ...
        tagList(
          ...
          lapply(cols, function(col) {
            if (
              !is.null(current$datetime) && identical(col, current$datetime)
            ) {
              return(NULL)
            }
            par_sel <- if (!is.null(current$params[[col]])) {
              current$params[[col]]$id
            } else {
              NULL
            }
            conv_sel <- if (!is.null(current$params[[col]])) {
              current$params[[col]]$conv
            } else {
              1
            }
            fluidRow(
              column(
                8,
                selectizeInput(
                  ns(paste0("param_", col)),
                  sprintf("Parameter for column '%s'", col),
                  choices = stats::setNames(
                    params()$parameter_id,
                    params()$param_name
                  ),
      ...
      if (input$file_format == "long") {
        ...
        param_list <- setNames(
          lapply(pnames, function(p) {
            key <- gsub("[^A-Za-z0-9]", "_", as.character(p))
            list(
              id = input[[paste0("param_", key)]],
              conv = input[[paste0("conv_", key)]]
            )
          }),
          pnames
        )
    ...
    observeEvent(input$add_row, {
      next_sample <- ifelse(
        nrow(data$df) == 0,
        1,
        max(data$df$sample, na.rm = TRUE) + 1
      )
      data$df <- rbind(
        data$df,
        data.frame(
    ...
    observeEvent(
      ...
      ),
      {
        req(file_data())
        df <- file_data()
        mapping <- if (nzchar(input$mapping_select)) {
          mappings()[[input$mapping_select]]
        } else {
          NULL
        }
        if (!is.null(mapping)) {
          if (identical(mapping$format, "long")) {
            ...
            df$parameter_id <- vapply(
              as.character(df[[param_col]]),
              function(p) mapping$params[[p]]$id,
              numeric(1)
            )
            conv <- vapply(
              as.character(df[[param_col]]),
              function(p) mapping$params[[p]]$conv,
              numeric(1)
            )
            data$df <- data.frame(
              sample = df$row_id,
            ...
            long$parameter_id <- vapply(
              long$pcol,
              function(pc) mapping$params[[pc]]$id,
              numeric(1)
            )
            conv <- vapply(
              long$pcol,
              function(pc) mapping$params[[pc]]$conv,
              numeric(1)
            )
            data$df <- data.frame(
              sample = long$row_id,
    ...
    insertDoc <- function(file) {
      ...
      id <- DBI::dbGetQuery(
        ...
        params = list(file$name, paste("Uploaded", file$name), ext, list(bin))
      )$document_id
      return(id)
    }
