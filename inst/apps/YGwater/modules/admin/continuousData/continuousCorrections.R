  ...
  tagList(
    ...
    page_fluid(
      ...
      accordion(
        ...
        accordion_panel(
          id = ns("existing_corrections_panel"),
...
continuousCorrections <- function(id, language) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      updateSelectizeInput(
        ...
        choices = stats::setNames(
          moduleData$types$correction_type_id,
          moduleData$types$correction_type
        ),
        server = TRUE
      )
    })
    ...
    observeEvent(
      ...
      {
        row <- moduleData$types[
          moduleData$types$correction_type_id == input$correction_type,
        ]
        if (nrow(row) == 0) {
          return()
        }
        ...
        if (!is.na(row$value2)) {
          ...
          updateNumericInput(
            session,
            "value2",
            label = paste0(row$value2_description, " (optional)")
          )
        }
    ...
    output$ts_table <- DT::renderDT({
      DT::datatable(
        ...
        options = list(
          columnDefs = list(
            list(targets = 0, visible = FALSE) #Hides the timeseries_id column. Column index numbers start at 0 here!!!
          ),
          scrollX = TRUE,
    ...
    observe({
      ...
      if (nrow(existing_corrections()) == 0) {
        output$no_corrections <- renderText(
        ...
        sub <- existing_corrections()[,
          colSums(is.na(existing_corrections())) < nrow(existing_corrections())
        ]
        output$existing_corrections <- DT::renderDT({
          DT::datatable(
            ...
            options = list(
              columnDefs = list(
                list(targets = 0, visible = FALSE) #Hides the correction_id column. Column index numbers start at 0 here!!!
              ),
              scrollX = TRUE,
              initComplete = htmlwidgets::JS(
                "function(settings, json) {",
