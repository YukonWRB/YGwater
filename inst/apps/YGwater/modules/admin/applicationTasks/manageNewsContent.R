    ...
    tabsetPanel(
      ...
      tabPanel(
        ...
        selectInput(
          ns("content_type"),
          "Content type",
          choices = c("text", "image")
        ),
        selectInput(
          ns("content_id"),
          "Content ID (reference name)",
          choices = NULL
        ),
        div(
...
manageNewsContent <- function(id, language) {
  moduleServer(id, function(input, output, session) {
    ...
    if (!check_text$can_insert) {
      shinyjs::disable("save_text")
    }

    check_image <- DBI::dbGetQuery(
      session$userData$AquaCache,
      "SELECT has_table_privilege(current_user, 'application.images', 'INSERT') AS can_insert"
    )
    if (!check_image$can_insert) {
      shinyjs::disable("save_image")
    }

    check_page <- DBI::dbGetQuery(
      session$userData$AquaCache,
    ...
    observeEvent(input$text_table_cell_edit, {
      ...
      if (col_name == "id") {
        ...
        DBI::dbExecute(
          session$userData$AquaCache,
          sprintf(
            "UPDATE application.text SET %s = $1 WHERE id = $2",
            col_name
          ),
          params = list(v, old_id)
        )
    ...
    observeEvent(input$image_table_cell_edit, {
      ...
      if (col_name == "id") {
        ...
        DBI::dbExecute(
          session$userData$AquaCache,
          sprintf(
            "UPDATE application.images SET %s = $1 WHERE id = $2",
            col_name
          ),
          params = list(v, old_id)
        )
