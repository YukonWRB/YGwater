...
feedback_ui <- function(id) {
  ...
  page_fluid(
    ...
    textAreaInput(ns("bugs"), "Did you encounter any bugs? If yes, please describe what the bug was and how it was triggered.", "", rows = 3),
    textAreaInput(ns("ui_comment"), "Do you have any comments or concerns about the user interface in general?", "", rows = 3),
    textAreaInput(ns("module_comment"), "Do you have comments about any particular module (page)?", "", rows = 3),
    radioButtons(ns("got_data"), "Did the app give you the data or visualization you were seeking?", choices = c("Yes", "No"), selected = "Yes"),
    textAreaInput(ns("missing"), "What was missing?", "", rows = 3),
    textAreInput(ns("addtional_comments"), "Do you have any additional comments?", "", rows = 3),
...
feedback <- function(id) {
  ...
  output$intro_text <- renderHTML({
    "Welcome to the user testing feedback page! We appreciate your input to help us improve the Water Data Explorer. Please fill out the form below to share your experience.<br>NOTE: this page will be deleted once internal and external user testing is complete."
  })
  ...
  observeEvent(input$submit, {
    info <- data.frame(
      ...
      user = if (nchar(input$user) > 0) input$user else "NOT SPECIFIED",
