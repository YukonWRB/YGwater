...
imgTableView <- function(id, language) {
  moduleServer(id, function(input, output, session) {
    ...
    imgs <- reactiveValues(
      ...
      ), # Note that this is added to later on if the user's date range is beyond 2 weeks ago; propagate any edits done to this query!
      img_min = Sys.Date() - 14,
    ...
    output$page <- renderUI({
      ...
      loc_choices <- c(
        loc_choices[1],
        loc_choices[-1][order(names(loc_choices)[-1])]
      )
    ...
    table_data <- reactive({
      ...
      data.table::setnames(
        tbl,
        c(
          tr("datetime_utc_offset", language$language),
    ...
    observe({
      ...
      out_tbl <- DT::datatable(
        ...
        options = list(
          ...
          columnDefs = list(
            list(targets = c(2, 3), visible = FALSE), #Hides the image_id and sort.dt columns
            list(targets = 1, orderData = 4),
            list(
              targets = 1,
