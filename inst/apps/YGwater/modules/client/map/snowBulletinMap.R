...
record_cutoff_years <- 1 # Only show stations with data within this many years
...
calculate_historic_daily_median <- function(
  ...
  if ("datetime" %in% names(ts)) {
    ...
    if (p > 0) {
      ...
      for (g in ug) {
        ...
        for (j_idx in seq_along(idx_g)) {
          ...
          if (!is.null(lookback_year)) {
            prev_idx <- idx_g[which(yg < yg[j_idx] & yg >= lookback_year)]
          } else {
          ...
          if (length(prev_idx) > 0) {
            ...
            if (requireNamespace("matrixStats", quietly = TRUE)) {
              hist_mat[j, ] <- matrixStats::colMedians(prev_block, na.rm = TRUE)
            } else {
              hist_mat[j, ] <- apply(prev_block, 2, stats::median, na.rm = TRUE)
            }
          }
        }
      }
      ...
      hist_df[station_cols] <- as.data.frame(hist_mat, stringsAsFactors = FALSE)
      rel_df[station_cols] <- as.data.frame(rel_mat, stringsAsFactors = FALSE)
    }
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$map <- leaflet::renderLeaflet({
      ...
      leaflet::leaflet(
        ...
        # --- SWE layers (rendered on first load) ---
        leaflet::addPolygons(
          ...
          label = ~ lapply(
            paste0(
              name,
              "<br>SWE: ",
              paste0(round(value_to_show %||% NA_real_, 1), "% of normal")
            ),
            htmltools::HTML
          ),
        ...
        # --- Add base layers (Boundary, Roads, Communities) on top ---
        # Boundary
        {
          if (!is.null(base_data$shapefiles$yukon)) {
            leaflet::addPolygons(
        ...
        # Communities and labels
        {
          if (!is.null(base_data$shapefiles$communities)) {
            ...
            . <- leaflet::addLabelOnlyMarkers(
              ...
              data = base_data$shapefiles$communities,
              label = ~ {
                nms <- names(base_data$shapefiles$communities)
                if ("name" %in% nms) {
                  as.character(base_data$shapefiles$communities$name)
                } else if ("location" %in% nms) {
                  as.character(base_data$shapefiles$communities$location)
                } else if ("feature_name" %in% nms) {
                  as.character(base_data$shapefiles$communities$feature_name)
                } else if ("description" %in% nms) {
                  as.character(base_data$shapefiles$communities$description)
                } else {
                  rep("", nrow(base_data$shapefiles$communities))
                }
              },
        ...
        leaflet::addPolygons(
          ...
          label = ~ lapply(
            paste0(
              name,
              "<br>SWE: ",
              if (value_type == "relative_swe") {
                paste0(round(value_to_show %||% NA_real_, 1), "% of normal")
              } else {
                paste0(round(value_to_show %||% NA_real_, 1), " mm")
              }
            ),
            htmltools::HTML
          ),
