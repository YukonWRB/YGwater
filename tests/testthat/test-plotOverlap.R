...
test_that("continuous level plot is as expected for full year with character startDay and endDay", {
  ...
  dir <- paste0(tempdir(), "/plots")
  dir.create(dir)
  suppressWarnings(plotOverlap("09EA004", "water level", startDay = "2023-01-01", endDay = "2023-12-31", years = "2022", save_path = dir, historic_range = "last"))
  path <- list.files(dir, full.names = TRUE)
...
test_that("continuous flow plot is as expected for full year with numeric startDay and endDay", {
  ...
  dir <- paste0(tempdir(), "/plots")
  dir.create(dir)
  suppressWarnings(plotOverlap("09EA004", "water flow", startDay = 1, endDay = 365, years = "2022", save_path = dir, historic_range = "last"))
  path <- list.files(dir, full.names = TRUE)
...
test_that("depth plot works when overlaping new year, dates as numeric", {
  ...
  dir <- paste0(tempdir(), "/plots")
  dir.create(dir)
  suppressWarnings(plotOverlap("09AA-M1", "snow depth", startDay = 250, endDay = 150, years = "2022", save_path = dir, return_months = c(4,5), historic_range = "last"))
  path <- list.files(dir, full.names = TRUE)
...
test_that("french labels work with overlaping years", {
  ...
  dir <- paste0(tempdir(), "/plots")
  dir.create(dir)
  suppressWarnings(plotOverlap("09AA-M1", "snow depth", startDay = 250, endDay = 150, years = "2022", save_path = dir, return_months = c(4,5), historic_range = "last", language = "fr"))
  path <- list.files(dir, full.names = TRUE)
  file.rename(path, paste0(dir, "/depth1_fr.png"))
  expect_snapshot_file(paste0(dir, "/depth1_fr.png"))
  unlink(dir, recursive = TRUE)
})
