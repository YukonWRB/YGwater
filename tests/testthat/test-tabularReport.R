test_that("tabular report is created without error on current data", {
  ...
  expect_no_error({
    res <- suppressMessages(tabularReport(precip_locations = NULL, bridge_locations = NULL, save_path = path, archive_path = NULL, past = 7, con = con))
})
...
test_that("tabular report is created without error on historic data", {
  ...
  # Check the 'timeseries' table to make sure there's data for today.
  check <- DBI::dbGetQuery(con, "SELECT COUNT(*) FROM measurements_continuous_corrected WHERE DATE(datetime) = '2020-06-01'")[1,1]
  if (check == 0) {
  ...
  expect_no_error({
    res <- suppressMessages(tabularReport(precip_locations = NULL, bridge_locations = NULL, save_path = path, archive_path = NULL, past = 7, report_datetime = as.POSIXct("2020-06-01 12:00", tz = "UTC"), con = con))
  })
