test_that("continuous level plot is as expected for full year with numeric startDay and endDay when saved to a file", {
  ...
  dir.create(dir)
  plot <- suppressWarnings(hydrometContinuous("09EA004", "level", startDay = 1, endDay = 365, years = "2022", save_path = dir,  historic_range = "last"))
...
test_that("continuous level plot is as expected for full year with numeric startDay and endDay when output to console", {
  skip_on_cran()
  skip_on_ci()
  plot <- suppressWarnings(hydrometContinuous("09EA004", "level", startDay = 1, endDay = 365, years = "2022", historic_range = "last"))
...
test_that("continuous level plot is as expected for full year with character startDay and endDay", {
  ...
  dir <- paste0(tempdir(), "/plots")
  dir.create(dir)
  suppressWarnings(hydrometContinuous("09EA004", "level", startDay = "2023-01-01", endDay = "2023-12-31", years = "2022", save_path = dir, historic_range = "last"))
  path <- list.files(dir, full.names=TRUE)
...
test_that("continuous flow plot is as expected for full year with numeric startDay and endDay", {
  ...
  dir <- paste0(tempdir(), "/plots")
  dir.create(dir)
  suppressWarnings(hydrometContinuous("09EA004", "flow", startDay = 1, endDay = 365, years = "2022", save_path = dir, historic_range = "last"))
  path <- list.files(dir, full.names=TRUE)
...
test_that("continuous level plot is as expected for multiple years when output to console", {
  skip_on_cran()
  skip_on_ci()
  plot <- suppressWarnings(hydrometContinuous("09EA004", "level", startDay = 1, endDay = 365, years = c(2019,2020,2021,2022), historic_range = "last"))
...
test_that("returns can be for yrs > requested year", {
  skip_on_cran()
  skip_on_ci()
  plot <- suppressWarnings(hydrometContinuous("09EA004", "level", startDay = 1, endDay = 365, years = c(2018), historic_range = "all", return_max_year = 2022))
  vdiffr::expect_doppelganger("hist range > plotted year", plot)
})
