...
test_url <- paste0(
  ...
  gsub("-", "", Sys.Date() - 1),
  "/WXO-DD/model_hrdpa/2.5km/18/",
  gsub("-", "", Sys.Date() - 1),
  "T18Z_MSC_HRDPA_APCP-Accum6h_Sfc_RLatLon0.0225_PT0H.grib2"
)
# Check if the test URL can be downloaded
curl::curl_download(
  test_url,
  destfile = tempfile(fileext = ".grib2"),
  quiet = TRUE
)
...
res <- basinPrecip(
  "09EA004",
  start = Sys.time() - 60 * 60 * 48,
  end = Sys.time() - 60 * 60 * 6
)
